...
APP_NAME="mop"
REPO_URL="https://github.com/redesblock/mop"

: "${USE_SUDO:="true"}"
: "${MOP_INSTALL_DIR:="/usr/local/bin"}"
...
# check_installed_version checks which version of mop is installed and
# if it needs to be changed.
check_installed_version() {
  if [[ -f "${MOP_INSTALL_DIR}/${APP_NAME}" ]]; then
    local version=$(mop version 2>&1)
    if [[ "${version%-*}" == "${TAG#v}" ]]; then
      echo "mop ${version} is already ${DESIRED_VERSION:-latest}"
      return 0
    else
      echo "mop ${TAG} is available. Changing from version ${version}."
      return 1
    fi
  else
    return 1
...
# for that binary.
download_file() {
  MOP_DIST="mop-$OS-$ARCH"
  if [ "$OS" == "windows" ]; then
    MOP_DIST="mop-$OS-$ARCH.exe"
  fi
  DOWNLOAD_URL="$REPO_URL/releases/download/$TAG/$MOP_DIST"
  MOP_TMP_ROOT="$(mktemp -dt mop-binary-XXXXXX)"
  MOP_TMP_FILE="$MOP_TMP_ROOT/$MOP_DIST"
  if command -v curl &> /dev/null; then
    curl -SsL "$DOWNLOAD_URL" -o "$MOP_TMP_FILE"
  elif command -v wget &> /dev/null; then
    wget -q -O "$MOP_TMP_FILE" "$DOWNLOAD_URL"
  fi
}
...
# installs it.
install_file() {
  echo "Preparing to install $APP_NAME into ${MOP_INSTALL_DIR}"
  run_as_root chmod +x "$MOP_TMP_FILE"
  run_as_root cp "$MOP_TMP_FILE" "$MOP_INSTALL_DIR/$APP_NAME"
  echo "$APP_NAME installed into $MOP_INSTALL_DIR/$APP_NAME"
}
...
test_binary() {
  if ! command -v $APP_NAME &> /dev/null; then
    echo "$APP_NAME not found. Is $MOP_INSTALL_DIR on your "'$PATH?'
    exit 1
  fi
