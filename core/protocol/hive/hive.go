...
const (
	...
	cacheSize              = 100000
	bitsPerByte            = 8
	cachePrefix            = cluster.MaxBins / bitsPerByte // enough bytes (32 bits) to uniquely identify a peer
)

var (
	limitBurst = 4 * int(cluster.MaxBins)
...
type Service struct {
	...
	addressBook       address.GetPutter
	addPeersHandler   func(...cluster.Address)
	networkID         uint64
	logger            log.Logger
	metrics           metrics
...
func New(streamer p2p.StreamerPinger, addressbook address.GetPutter, networkID uint64, bootnode bool, allowPrivateCIDRs bool, logger log.Logger) (*Service, error) {
...
func (s *Service) BroadcastPeers(ctx context.Context, addressee cluster.Address, peers ...cluster.Address) error {
	max := maxBatchSize
...
func (s *Service) SetAddPeersHandler(h func(addr ...cluster.Address)) {
	s.addPeersHandler = h
}
...
func (s *Service) sendPeers(ctx context.Context, peer cluster.Address, peers []cluster.Address) (err error) {
	addr, err := s.addressBook.Get(peer)
	if err != nil && !errors.Is(err, address.ErrNotFound) {
		return err
	}
	...
	for _, p := range peers {
		...
		if err != nil {
			if err == address.ErrNotFound {
				s.logger.Debug("broadcast peers; peer not found in the address, skipping...", "peer_address", p)
				continue
			}
			return err
		}
...
func (s *Service) checkAndAddPeers(ctx context.Context, peers pb.Peers) {
	...
	var peersToAdd []cluster.Address
	mtx := sync.Mutex{}
	wg := sync.WaitGroup{}

	for _, p := range peers.Peers {

		overlay := cluster.NewAddress(p.Overlay)
		...
		go func(newPeer *pb.MopAddress, cacheOverlay string) {
			...
			mopAddress := mop.Address{
				Overlay:     cluster.NewAddress(newPeer.Overlay),
