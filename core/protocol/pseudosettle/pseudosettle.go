...
import (
	...
	pb "github.com/redesblock/mop/core/protocol/pseudosettle/pb"
...
func totalKey(peer cluster.Address, prefix string) string {
	return fmt.Sprintf("%v%v", prefix, peer.String())
}

func totalKeyPeer(key []byte, prefix string) (peer cluster.Address, err error) {
	k := string(key)
	...
	return cluster.ParseHexAddress(split[1])
}
...
func (s *Service) peerAllowance(peer cluster.Address, fullNode bool) (limit *big.Int, stamp int64, err error) {
