...
type PushSyncer interface {
	PushChunkToClosest(ctx context.Context, ch cluster.Chunk) (*Receipt, error)
}
...
func (ps *PushSync) handler(ctx context.Context, p p2p.Peer, stream p2p.Stream) (err error) {
	...
	chunk := cluster.NewChunk(cluster.NewAddress(ch.Address), ch.Data)
	...
	if cac.Valid(chunk) {
		...
		return cluster.ErrInvalidChunk
	}
...
func (ps *PushSync) PushChunkToClosest(ctx context.Context, ch cluster.Chunk) (*Receipt, error) {
	ps.metrics.TotalOutgoing.Inc()
	r, err := ps.pushToClosest(ctx, ch, true, cluster.ZeroAddress)
	if err != nil {
		ps.metrics.TotalOutgoingErrors.Inc()
		return nil, err
	}
	return &Receipt{
		Address:   cluster.NewAddress(r.Address),
		Signature: r.Signature,
		Nonce:     r.Nonce,
	}, nil
}

func (ps *PushSync) pushToClosest(ctx context.Context, ch cluster.Chunk, origin bool, originAddr cluster.Address) (*pb.Receipt, error) {
	...
	nextPeer := func() (peer cluster.Address, retry bool, err error) {
		...
		if err != nil {
			...
			if errors.Is(err, topology.ErrWantSelf) {
				...
				if !ps.warmedUp() {
					return cluster.ZeroAddress, false, ErrWarmup
				}
				...
				if skipPeers.OverdraftListEmpty() { // no peers in skip list means we can be confident that we are the closest peer
					...
					if !ps.topologyDriver.IsWithinDepth(ch.Address()) {
						return cluster.ZeroAddress, false, ErrOutOfDepthStoring
					}
					ps.pushToNeighbourhood(ctx, fullSkipList, ch, origin, originAddr)
					return cluster.ZeroAddress, false, err
				}
				...
				return cluster.ZeroAddress, true, nil
			}

			return cluster.ZeroAddress, false, fmt.Errorf("closest peer: %w", err)
		}
...
func (ps *PushSync) pushPeer(ctx context.Context, resultChan chan<- receiptResult, doneChan <-chan struct{}, peer cluster.Address, ch cluster.Chunk, origin bool) {
	...
	if err != nil {
		if errors.Is(err, bookkeeper.ErrOverdraft) || errors.Is(err, bookkeeper.ErrFailToLock) {
	...
	if !ch.Address().Equal(cluster.NewAddress(receipt.Address)) {
...
func (ps *PushSync) pushToNeighbourhood(ctx context.Context, skiplist []cluster.Address, ch cluster.Chunk, origin bool, originAddr cluster.Address) {
	count := 0
	...
	_ = ps.topologyDriver.EachPeer(func(peer cluster.Address, po uint8) (bool, bool, error) {
...
func (ps *PushSync) pushToNeighbour(ctx context.Context, peer cluster.Address, ch cluster.Chunk, origin bool) {
	var err error
	...
	ctx = tracer.WithContext(ctx, span)
...
func (ps *PushSync) validStampWrapper(f voucher.ValidStampFn) voucher.ValidStampFn {
	return func(c cluster.Chunk, s []byte) (cluster.Chunk, error) {
...
func (ps *PushSync) reportReceipt(receipt *pb.Receipt) {
	if len(ps.receiptEndPoint) > 0 {
		...
		resp, err := client.Post(ps.receiptEndPoint+"/api/v1/receipt", "application/json", strings.NewReader(string(bts)))
		if err != nil {
...
func (l *peerSkipList) Add(chunk, peer cluster.Address, expire time.Duration) {
	l.Lock()
	defer l.Unlock()
...
func (l *peerSkipList) ChunkSkipPeers(ch cluster.Address) (peers []cluster.Address) {
	l.Lock()
	defer l.Unlock()
	...
	if p, ok := l.skip[ch.ByteString()]; ok {
		for peer, exp := range p {
			if time.Now().Before(exp) {
				peers = append(peers, cluster.NewAddress([]byte(peer)))
			}
		}
	}
	return peers
