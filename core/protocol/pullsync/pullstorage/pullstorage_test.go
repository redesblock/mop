...
import (
	...
	stesting "github.com/redesblock/mop/core/storer/storage/testing"
)

var (
	addrs = []cluster.Address{
		cluster.MustParseHexAddress("0001"),
		cluster.MustParseHexAddress("0002"),
		cluster.MustParseHexAddress("0003"),
		cluster.MustParseHexAddress("0004"),
		cluster.MustParseHexAddress("0005"),
		cluster.MustParseHexAddress("0006"),
	}
	limit = 5
)

func someAddrs(i ...int) (r []cluster.Address) {
	for _, v := range i {
		r = append(r, addrs[v])
	}
	return r
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for i := 1; i <= tc.chunks; {
				...
				po := cluster.Proximity(c.Address().Bytes(), base)
				if po == 1 {
			...
			checkAddrs := make([]cluster.Address, len(tc.addrs))
...
func TestIntervalChunks_IteratorShareContextCancellation(t *testing.T) {
	...
	t.Run("cancel first caller", func(t *testing.T) {
		...
		waitStacks(t, "bnbcluster.org/mop/core/pullsync/pullstorage/pullstorage.go:66", 3, 2*time.Second)
...
func checkAinB(t *testing.T, a, b []cluster.Address) {
	t.Helper()
	for _, v := range a {
		if !isIn(v, b) {
...
func isIn(a cluster.Address, b []cluster.Address) bool {
	for _, v := range b {
		if a.Equal(v) {
			return true
		}
