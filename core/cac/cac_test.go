...
func TestNewCAC(t *testing.T) {
	...
	address := flock.MustParseHexAddress(bmtHashOfData)

	expectedSpan := make([]byte, flock.SpanSize)
...
func TestChunkInvariants(t *testing.T) {
	chunkerFunc := []struct {
		name    string
		chunker func(data []byte) (flock.Chunk, error)
	}{
		{
			name:    "new cac",
			chunker: cac.New,
	...
	for _, f := range chunkerFunc {
		for _, cc := range []struct {
			...
			},
			{
				...
				data:    []byte(strings.Repeat("a", flock.ChunkSize+flock.SpanSize+1)),
...
func TestValid(t *testing.T) {
	...
	foo := "foo"
	fooLength := len(foo)
	fooBytes := make([]byte, flock.SpanSize+fooLength)
	binary.LittleEndian.PutUint64(fooBytes, uint64(fooLength))
	copy(fooBytes[flock.SpanSize:], foo)
	ch := flock.NewChunk(address, fooBytes)
	if !cac.Valid(ch) {
