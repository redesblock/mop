// Package syncer provides orchestration logic for
...
type prover interface {
	Prove(context.Context, cluster.Address, uint64) ([]byte, error)
}
...
func (c *ChainSyncer) manage() {
	...
	for {
		...
		items = 0
		positives = 0
		var seen []cluster.Address
		cctx, cancelF := context.WithTimeout(ctx, defaultPollEvery)
		_ = c.peerIterator.EachPeer(func(p cluster.Address, _ uint8) (bool, bool, error) {
			seen = append(seen, p)
			wg.Add(1)
			items++
			go func(peer cluster.Address) {
				defer wg.Done()
...
func (c *ChainSyncer) Close() error {
	...
	case <-time.After(5 * time.Second):
		return errors.New("syncer shutting down with running goroutines")
	}
	return nil
}
