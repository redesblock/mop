...
type Joiner interface {
	...
	IterateChunkAddresses(cluster.AddressIterFunc) error
...
// When EOF is received and splitting is done, the resulting Cluster Address is returned.
type Splitter interface {
	Split(ctx context.Context, dataIn io.ReadCloser, dataLength int64, toEncrypt bool) (addr cluster.Address, err error)
}
