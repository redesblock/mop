...
var (
	dataWrites = [][]int{
		{cluster.ChunkSize - 2},                           // short
		{cluster.ChunkSize - 2, 4},                        // short, over
		{cluster.ChunkSize - 2, 4, cluster.ChunkSize - 6}, // short, over, short
		{cluster.ChunkSize - 2, 4, cluster.ChunkSize - 4}, // short, over, onononon
		{cluster.ChunkSize, 2, cluster.ChunkSize - 4},     // on, short, short
		{cluster.ChunkSize, 2, cluster.ChunkSize - 2},     // on, short, on
		{cluster.ChunkSize, 2, cluster.ChunkSize},         // on, short, over
		{cluster.ChunkSize, 2, cluster.ChunkSize - 2, 4},  // on, short, on, short
		{cluster.ChunkSize, cluster.ChunkSize},            // on, on
	}
)
...
func TestCopyBuffer(t *testing.T) {
	...
	dataSizes := []int{
		...
		cluster.ChunkSize + 1,
		cluster.ChunkSize * 2,
		cluster.ChunkSize*2 + 3,
		cluster.ChunkSize * 5,
		cluster.ChunkSize*5 + 3,
		cluster.ChunkSize * 17,
		cluster.ChunkSize*17 + 3,
	}
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("buf_%-4d/data_size_%d", tc.readBufferSize, tc.dataSize), func(t *testing.T) {
			...
			go func() {
				...
				buf := make([]byte, cluster.ChunkSize)
