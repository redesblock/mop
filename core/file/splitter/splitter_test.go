...
func TestSplitSingleChunk(t *testing.T) {
	...
	testData, err := g.SequentialBytes(flock.ChunkSize)
	if err != nil {
		t.Fatal(err)
	}
	...
	testHashAddress := flock.MustParseHexAddress(testHashHex)
...
func TestSplitThreeLevels(t *testing.T) {
	...
	g := mockbytes.New(0, mockbytes.MockTypeStandard).WithModulus(255)
	testData, err := g.SequentialBytes(flock.ChunkSize * 128)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < flock.ChunkSize; i += flock.SectionSize {
		dataAddressBytes := rootData[i : i+flock.SectionSize]
		dataAddress := flock.NewAddress(dataAddressBytes)
...
func TestUnalignedSplit(t *testing.T) {
	...
	var (
		dataLen       int64 = flock.ChunkSize*2 + 32
	...
	writeSizes := []int{flock.ChunkSize - 40, 40 + 32, flock.ChunkSize}
	...
	select {
	case addr := <-doneC:
		expectAddr := flock.MustParseHexAddress(expectAddrHex)
