...
func TestSplitSingleChunk(t *testing.T) {
	...
	testData, err := g.SequentialBytes(cluster.ChunkSize)
	if err != nil {
		t.Fatal(err)
	}
	...
	testHashAddress := cluster.MustParseHexAddress(testHashHex)
...
func TestSplitThreeLevels(t *testing.T) {
	...
	g := mockbytes.New(0, mockbytes.MockTypeStandard).WithModulus(255)
	testData, err := g.SequentialBytes(cluster.ChunkSize * 128)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < cluster.ChunkSize; i += cluster.SectionSize {
		dataAddressBytes := rootData[i : i+cluster.SectionSize]
		dataAddress := cluster.NewAddress(dataAddressBytes)
...
func TestUnalignedSplit(t *testing.T) {
	...
	// test vector taken from core/file/testing/vector.go
	var (
		dataLen       int64 = cluster.ChunkSize*2 + 32
	...
	doneC := make(chan cluster.Address)
	...
	writeSizes := []int{cluster.ChunkSize - 40, 40 + 32, cluster.ChunkSize}
	...
	select {
	case addr := <-doneC:
		expectAddr := cluster.MustParseHexAddress(expectAddrHex)
...
goos: linux
goarch: amd64
core: github.com/redesblock/mop/core/file/splitter
BenchmarkSplitter
