...
type Putter interface {
	Put(context.Context, cluster.Chunk) ([]bool, error)
}
...
// return the ClusterHash of the data.
//
...
func NewSimpleSplitterJob(ctx context.Context, putter Putter, spanLength int64, toEncrypt bool) *SimpleSplitterJob {
	...
	return &SimpleSplitterJob{
		...
		buffer:     make([]byte, cluster.ChunkWithSpanSize*levelBufferLimit*2), // double size as temp workaround for weak calculation of needed buffer space
		tag:        mctx.GetTag(ctx),
		toEncrypt:  toEncrypt,
		refSize:    refSize,
	}
...
func (j *SimpleSplitterJob) Write(b []byte) (int, error) {
	if len(b) > cluster.ChunkSize {
		return 0, fmt.Errorf("Write must be called with a maximum of %d bytes", cluster.ChunkSize)
	}
...
// Sum returns the Cluster hash of the data.
...
func (s *SimpleSplitterJob) writeToLevel(lvl int, data []byte) error {
	...
	if s.cursors[lvl]-s.cursors[lvl+1] == cluster.ChunkSize {
...
func (s *SimpleSplitterJob) sumLevel(lvl int) ([]byte, error) {
	s.sumCounts[lvl]++
	spanSize := file.Spans[lvl] * cluster.ChunkSize
	span := (s.length-1)%spanSize + 1

	var chunkData []byte

	head := make([]byte, cluster.SpanSize)
...
func (s *SimpleSplitterJob) digest() []byte {
	if s.toEncrypt {
		return s.buffer[:cluster.SectionSize*2]
	} else {
		return s.buffer[:cluster.SectionSize]
	}
}
...
func (s *SimpleSplitterJob) hashUnfinished() error {
	if s.length%cluster.ChunkSize != 0 {
...
func (s *SimpleSplitterJob) moveDanglingChunk() error {
	...
	targetLevel := file.Levels(s.length, cluster.SectionSize, cluster.Branches)
...
func (s *SimpleSplitterJob) newSpanEncryption(key encryption.Key) encryption.Interface {
	return encryption.New(key, 0, uint32(cluster.ChunkSize/s.refSize), sha3.NewLegacyKeccak256)
}
