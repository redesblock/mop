...
type putWrapper struct {
	putter func(context.Context, cluster.Chunk) ([]bool, error)
}

func (p putWrapper) Put(ctx context.Context, ch cluster.Chunk) ([]bool, error) {
...
func NewSimpleSplitter(storePutter storage.Putter, mode storage.ModePut) file.Splitter {
	return &simpleSplitter{
		putter: putWrapper{
			putter: func(ctx context.Context, ch cluster.Chunk) ([]bool, error) {
...
//
// It returns the Clusterhash of the data.
func (s *simpleSplitter) Split(ctx context.Context, r io.ReadCloser, dataLength int64, toEncrypt bool) (addr cluster.Address, err error) {
	...
	for !eof {
		...
		if err != nil {
			if err == io.EOF {
				if total < dataLength {
					return cluster.ZeroAddress, fmt.Errorf("splitter only received %d bytes of data, expected %d bytes", total+int64(c), dataLength)
				}
				eof = true
				continue
			} else {
		...
		if cc < c {
			return cluster.ZeroAddress, fmt.Errorf("write count to file hasher component %d does not match read count %d", cc, c)
		}
	}
