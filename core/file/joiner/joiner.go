...
func New(ctx context.Context, getter storage.Getter, address cluster.Address) (file.Joiner, int64, error) {
	...
	span := int64(binary.LittleEndian.Uint64(chunkData[:cluster.SpanSize]))
	...
	j := &joiner{
		...
		getter:    getter,
		span:      span,
		rootData:  chunkData[cluster.SpanSize:],
	}
...
func (j *joiner) ReadAt(buffer []byte, off int64) (read int, err error) {
	// since offset is int64 and cluster spans are uint64 it means we cannot seek beyond int64 max value
	if off >= j.span {
		return 0, io.EOF
	}
...
func (j *joiner) readAtOffset(b, data []byte, cur, subTrieSize, off, bufferOffset, bytesToRead int64, bytesRead *int64, eg *errgroup.Group) {
	...
	for cursor := 0; cursor < len(data); cursor += j.refLength {
		...
		address := cluster.NewAddress(data[cursor : cursor+j.refLength])
		...
		func(address cluster.Address, b []byte, cur, subTrieSize, off, bufferOffset, bytesToRead, subtrieSpanLimit int64) {
			eg.Go(func() error {
...
func subtrieSection(data []byte, startIdx, refLen int, subtrieSize int64) int64 {
	...
	var (
		...
		branching  = int64(cluster.Branches)   // branching factor is chunkSize divided by reference length
...
func (j *joiner) IterateChunkAddresses(fn cluster.AddressIterFunc) error {
	// report root address
	err := fn(j.addr)
	if err != nil {
		return err
...
func (j *joiner) processChunkAddresses(ctx context.Context, fn cluster.AddressIterFunc, data []byte, subTrieSize int64) error {
	// we are at a leaf data chunk
	if subTrieSize <= int64(len(data)) {
		return nil
	}
	...
	for cursor := 0; cursor < len(data); cursor += j.refLength {
		...
		var reportAddr cluster.Address
		address := cluster.NewAddress(ref)
		if len(ref) == encryption.ReferenceSize {
			reportAddr = cluster.NewAddress(ref[:cluster.HashSize])
		} else {
			reportAddr = cluster.NewAddress(ref)
		}
		...
		sec := subtrieSection(data, cursor, j.refLength, subTrieSize)
		if sec <= cluster.ChunkSize {
			continue
		}

		func(address cluster.Address, eg *errgroup.Group) {
			wg.Add(1)
