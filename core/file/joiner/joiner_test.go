...
import (
	...
	"github.com/redesblock/mop/core/chunk/encryption/store"
...
func TestJoiner_ErrReferenceLength(t *testing.T) {
	...
	_, _, err := joiner.New(context.Background(), store, cluster.ZeroAddress)

	if !errors.Is(err, storage.ErrReferenceLength) {
		t.Fatalf("expected ErrReferenceLength %x but got %v", cluster.ZeroAddress, err)
	}
}
...
func TestJoinerSingleChunk(t *testing.T) {
	...
	mockAddr := cluster.MustParseHexAddress(mockAddrHex)
	mockData := []byte("foo")
	mockDataLengthBytes := make([]byte, 8)
	mockDataLengthBytes[0] = 0x03
	mockChunk := cluster.NewChunk(mockAddr, append(mockDataLengthBytes, mockData...))
...
func TestJoinerWithReference(t *testing.T) {
	...
	if l != int64(cluster.ChunkSize*2) {
		t.Fatalf("expected join data length %d, got %d", cluster.ChunkSize*2, l)
	}

	resultBuffer := make([]byte, cluster.ChunkSize)
...
func TestJoinerMalformed(t *testing.T) {
	...
	binary.LittleEndian.PutUint64(rootChunkData[:8], uint64(cluster.ChunkSize*2))
...
func TestSeek(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name: "one chunk",
			size: cluster.ChunkSize,
		},
		{
			name: "a few chunks",
			size: 10 * cluster.ChunkSize,
		},
		{
			name: "a few chunks and a change",
			size: 10*cluster.ChunkSize + 84,
		},
		{
			name: "a few chunks more",
			size: 2*cluster.ChunkSize*cluster.ChunkSize + 1000,
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			validateRead := func(t *testing.T, name string, i int) {
				...
				got := make([]byte, cluster.ChunkSize)
...
func TestJoinerReadAt(t *testing.T) {
	...
	b := make([]byte, cluster.ChunkSize)
	_, err = j.ReadAt(b, cluster.ChunkSize)
	if err != nil {
		t.Fatal(err)
	}
...
func TestJoinerOneLevel(t *testing.T) {
	...
	outBuffer := make([]byte, cluster.ChunkSize)
	c, err := j.Read(outBuffer)
	if err != nil {
		t.Fatal(err)
	}
	if c != cluster.ChunkSize {
		t.Fatalf("expected firstchunk read count %d, got %d", cluster.ChunkSize, c)
	}
	...
	if c != cluster.ChunkSize {
		t.Fatalf("expected secondchunk read count %d, got %d", cluster.ChunkSize, c)
	}
...
func TestJoinerTwoLevelsAcrossChunk(t *testing.T) {
	...
	rootChunk := filetest.GenerateTestRandomFileChunk(cluster.ZeroAddress, cluster.ChunkSize*cluster.Branches+42, cluster.SectionSize*2)
	_, err := store.Put(ctx, storage.ModePutUpload, rootChunk)
	if err != nil {
		t.Fatal(err)
	}
	...
	firstAddress := cluster.NewAddress(rootChunk.Data()[8 : cluster.SectionSize+8])
	firstChunk := filetest.GenerateTestRandomFileChunk(firstAddress, cluster.ChunkSize*cluster.Branches, cluster.ChunkSize)
	_, err = store.Put(ctx, storage.ModePutUpload, firstChunk)
	if err != nil {
		t.Fatal(err)
	}
	...
	secondAddress := cluster.NewAddress(rootChunk.Data()[cluster.SectionSize+8:])
	secondChunk := filetest.GenerateTestRandomFileChunk(secondAddress, 42, cluster.SectionSize)
	_, err = store.Put(ctx, storage.ModePutUpload, secondChunk)
	if err != nil {
		t.Fatal(err)
	}
	...
	cursor := 8
	for i := 0; i < cluster.Branches; i++ {
		chunkAddressBytes := firstChunk.Data()[cursor : cursor+cluster.SectionSize]
		chunkAddress := cluster.NewAddress(chunkAddressBytes)
		ch := filetest.GenerateTestRandomFileChunk(chunkAddress, cluster.ChunkSize, cluster.ChunkSize)
		_, err := store.Put(ctx, storage.ModePutUpload, ch)
		if err != nil {
			t.Fatal(err)
		}
		cursor += cluster.SectionSize
	}
	...
	for i := 0; i < cluster.Branches; i++ {
		...
		if c != cluster.ChunkSize {
			t.Fatalf("chunk %d expected read %d bytes; got %d", i, cluster.ChunkSize, c)
		}
	}
	c, err := j.Read(b)
	if err != nil {
...
func TestJoinerIterateChunkAddresses(t *testing.T) {
	...
	err = j.IterateChunkAddresses(func(addr cluster.Address) error {
