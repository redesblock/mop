...
const (
	maxBufferSize = cluster.ChunkSize * 2
)
...
func (c *ChunkPipe) Write(b []byte) (int, error) {
	...
	for {
		...
		if c.cursor >= cluster.ChunkSize {
			// NOTE: the Write method contract requires all sent data to be
			// written before returning (without error)
			written, err := c.writer.Write(c.data[:cluster.ChunkSize])
			if err != nil {
				return nw, err
			}
			if cluster.ChunkSize != written {
				return nw, io.ErrShortWrite
			}

			c.cursor -= cluster.ChunkSize

			copy(c.data, c.data[cluster.ChunkSize:])
		}
	}
