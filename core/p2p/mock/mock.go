...
type Service struct {
	...
	disconnectFunc        func(overlay cluster.Address, reason string) error
...
func WithDisconnectFunc(f func(overlay cluster.Address, reason string) error) Option {
	return optionFunc(func(s *Service) {
		s.disconnectFunc = f
	})
}
...
func WithBlocklistFunc(f func(cluster.Address, time.Duration, string) error) Option {
	return optionFunc(func(s *Service) {
		s.blocklistFunc = f
	})
}
...
func (s *Service) Disconnect(overlay cluster.Address, reason string) error {
