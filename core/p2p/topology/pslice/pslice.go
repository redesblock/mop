...
type PSlice struct {
	peers     [][]cluster.Address // the slice of peers
	baseBytes []byte
	mu        sync.RWMutex
	maxBins   int
}
...
func New(maxBins int, base cluster.Address) *PSlice {
	return &PSlice{
		peers:     make([][]cluster.Address, maxBins),
...
func (s *PSlice) Add(addrs ...cluster.Address) {
	...
	for i, count := range binChange {
		...
		if count > 0 && cap(peers) < len(peers)+count {
			newPeers := make([]cluster.Address, len(peers), len(peers)+count)
			copy(newPeers, peers)
			s.peers[i] = newPeers
		}
	}
...
func (s *PSlice) BinPeers(bin uint8) []cluster.Address {
	...
	ret := make([]cluster.Address, len(s.peers[bin]))
...
func (s *PSlice) Remove(addr cluster.Address) {
	...
	cpy := make([]cluster.Address, newLength)
...
func (s *PSlice) po(peer []byte) uint8 {
	po := cluster.Proximity(s.baseBytes, peer)
	if int(po) >= s.maxBins {
...
func (s *PSlice) index(addr cluster.Address, po uint8) (bool, int) {
