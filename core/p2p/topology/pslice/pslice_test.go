...
func TestAddRemove(t *testing.T) {
	...
	chkNotExists(t, ps, append([]cluster.Address{peers[1]}, peers[4:]...)...)

	// this should not move the last cursor
	ps.Add(peers[7])
	chkLen(t, ps, 4)
	chkExists(t, ps, peers[0], peers[2], peers[3], peers[7])
	chkNotExists(t, ps, append([]cluster.Address{peers[1]}, peers[4:7]...)...)

	ps.Add(peers[5])
	chkLen(t, ps, 5)
	chkExists(t, ps, peers[0], peers[2], peers[3], peers[5], peers[7])
	chkNotExists(t, ps, []cluster.Address{peers[1], peers[4], peers[6]}...)

	ps.Remove(peers[2])
	chkLen(t, ps, 4)
	chkExists(t, ps, peers[0], peers[3], peers[5], peers[7])
	chkNotExists(t, ps, []cluster.Address{peers[1], peers[2], peers[4], peers[6]}...)
...
func TestIteratorError(t *testing.T) {
	...
	f := func(p cluster.Address, _ uint8) (stop, jumpToNext bool, err error) {
...
func TestIterators(t *testing.T) {
	...
	peers := make([]cluster.Address, 4)
	for i := 0; i < 4; i++ {
		a := test.RandomAddressAt(base, i)
		peers[i] = a
	}

	testIterator(t, ps, false, false, 0, []cluster.Address{})
	testIteratorRev(t, ps, false, false, 0, []cluster.Address{})

	for _, v := range peers {
		ps.Add(v)
	}

	testIterator(t, ps, false, false, 4, []cluster.Address{peers[3], peers[2], peers[1], peers[0]})
	testIteratorRev(t, ps, false, false, 4, peers)

	ps.Remove(peers[2])
	testIterator(t, ps, false, false, 3, []cluster.Address{peers[3], peers[1], peers[0]})
	testIteratorRev(t, ps, false, false, 3, []cluster.Address{peers[0], peers[1], peers[3]})

	ps.Remove(peers[0])
	testIterator(t, ps, false, false, 2, []cluster.Address{peers[3], peers[1]})
	testIteratorRev(t, ps, false, false, 2, []cluster.Address{peers[1], peers[3]})

	ps.Remove(peers[3])
	testIterator(t, ps, false, false, 1, []cluster.Address{peers[1]})
	testIteratorRev(t, ps, false, false, 1, []cluster.Address{peers[1]})
...
func TestBinPeers(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.label, func(t *testing.T) {
			...
			binPeers := make([][]cluster.Address, len(tc.peersCount))
...
func isEqual(a, b []cluster.Address) bool {
...
func TestIteratorsJumpStop(t *testing.T) {
	...
	testIterator(t, ps, true, false, 4, []cluster.Address{peers[9], peers[6], peers[3], peers[0]})
	testIteratorRev(t, ps, true, false, 4, []cluster.Address{peers[0], peers[3], peers[6], peers[9]})

	// // check that the stop functionality works correctly
	testIterator(t, ps, true, true, 1, []cluster.Address{peers[9]})
	testIteratorRev(t, ps, true, true, 1, []cluster.Address{peers[0]})

}

func testIteratorRev(t *testing.T, ps *pslice.PSlice, skipNext, stop bool, iterations int, peerseq []cluster.Address) {
	t.Helper()
	i := 0
	f := func(p cluster.Address, po uint8) (bool, bool, error) {
		if i == iterations {
...
func testIterator(t *testing.T, ps *pslice.PSlice, skipNext, stop bool, iterations int, peerseq []cluster.Address) {
	t.Helper()
	i := 0
	f := func(p cluster.Address, po uint8) (bool, bool, error) {
		if i == iterations {
...
func chkExists(t *testing.T, ps *pslice.PSlice, addrs ...cluster.Address) {
	t.Helper()
	for _, a := range addrs {
		if !ps.Exists(a) {
...
func chkNotExists(t *testing.T, ps *pslice.PSlice, addrs ...cluster.Address) {
	t.Helper()
	for _, a := range addrs {
		if ps.Exists(a) {
...
func BenchmarkEachBin(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		_ = ps.EachBin(func(a cluster.Address, u uint8) (stop bool, jumpToNext bool, err error) {
...
func BenchmarkEachBinRev(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		_ = ps.EachBinRev(func(a cluster.Address, u uint8) (stop bool, jumpToNext bool, err error) {
			return false, false, nil
		})
	}
}
