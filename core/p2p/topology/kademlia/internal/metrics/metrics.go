...
type persistentCounters struct {
	PeerAddress       cluster.Address `json:"peerAddress"`
...
// peers specified be the cluster.Address.
...
func (c *Collector) Record(addr cluster.Address, rop ...RecordOp) {
...
// session.
func (c *Collector) Snapshot(t time.Time, addresses ...cluster.Address) map[string]*Snapshot {
...
func (c *Collector) IsUnreachable(addr cluster.Address) bool {
	val, ok := c.counters.Load(addr.ByteString())
	if !ok {
		return true
	}
...
// Inspect allows inspecting current snapshot for the given
// peer address by executing the inspection function.
func (c *Collector) Inspect(addr cluster.Address) *Snapshot {
	snapshots := c.Snapshot(time.Now(), addr)
	return snapshots[addr.ByteString()]
}

// Flush chainsync the dirty in memory counters for all peers by flushing their
