...
func generateCommonBinPrefixes(base cluster.Address, suffixLength int) [][]cluster.Address {
	...
	addr := cluster.MustParseHexAddress(base.String())
	addrBytes := addr.Bytes()
	_ = addrBytes

	binPrefixes := make([][]cluster.Address, int(cluster.MaxBins))

	// copy base address
	for i := range binPrefixes {
		binPrefixes[i] = make([]cluster.Address, bitCombinationsCount)
	}
	...
	for i := range binPrefixes {
		for j := range binPrefixes[i] {
			...
			binPrefixes[i][j] = cluster.NewAddress(pseudoAddrBytes)
		}
	}
