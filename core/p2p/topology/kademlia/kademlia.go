...
import (
	...
	"github.com/redesblock/mop/core/p2p/topology/discovery"
	im "github.com/redesblock/mop/core/p2p/topology/kademlia/internal/metrics"
	"github.com/redesblock/mop/core/p2p/topology/kademlia/internal/waitnext"
...
type (
	...
	sanctionedPeerFunc func(peer cluster.Address) bool
	pruneFunc          func(depth uint8)
	staticPeerFunc     func(peer cluster.Address) bool
	peerFilterFunc     func(peer cluster.Address) bool
)

var noopSanctionedPeerFn = func(_ cluster.Address) bool { return false }
...
// Kad is the Cluster forwarding kademlia implementation.
type Kad struct {
	base               cluster.Address     // this node's overlay address
	discovery          discovery.Driver    // the discovery driver
	addressBook        address.Interface   // address book to get underlays
	...
	commonBinPrefixes  [][]cluster.Address // list of address prefixes for each bin
...
func New(
	...
	if o.SaturationFunc == nil {
		...
		o.SaturationFunc = binSaturated(os, isStaticPeer(o.StaticNodes), k)
	}
	...
	k = &Kad{
		...
		commonBinPrefixes:  make([][]cluster.Address, int(cluster.MaxBins)),
		connectedPeers:     pslice.New(int(cluster.MaxBins), base),
		knownPeers:         pslice.New(int(cluster.MaxBins), base),
		...
		storageRadius:      cluster.MaxPO,
	}

	blocklistCallback := func(a cluster.Address) {
...
func (k *Kad) connectBalanced(wg *sync.WaitGroup, peerConnChan chan<- *peerConnInfo) {
	skipPeers := func(peer cluster.Address) bool {
		...
		for j := range k.commonBinPrefixes[i] {
			...
			default:
				...
				case peerConnChan <- &peerConnInfo{
					po:   cluster.Proximity(k.base.Bytes(), closestKnownPeer.Bytes()),
...
func (k *Kad) connectNeighbours(wg *sync.WaitGroup, peerConnChan chan<- *peerConnInfo) {
	...
	_ = k.knownPeers.EachBinRev(func(addr cluster.Address, po uint8) (bool, bool, error) {
		depth := k.NeighborhoodDepth()
...
func (k *Kad) connectionAttemptsHandler(ctx context.Context, wg *sync.WaitGroup, neighbourhoodChan, balanceChan <-chan *peerConnInfo) {
	connect := func(peer *peerConnInfo) {
		mopAddress, err := k.addressBook.Get(peer.addr)
		switch {
		...
		remove := func(peer *peerConnInfo) {
			...
			if err := k.addressBook.Remove(peer.addr); err != nil {
				k.logger.Debug("could not remove peer from address", "peer_address", peer.addr)
			}
		}

		switch err = k.connect(ctx, peer.addr, mopAddress.Underlay); {
		case errors.Is(err, p2p.ErrNetworkUnavailable):
			k.logger.Debug("network unavailable when reaching peer", "peer_overlay_address", peer.addr, "peer_underlay_address", mopAddress.Underlay)
			return
		case errors.Is(err, errPruneEntry):
			k.logger.Debug("dial to light node", "peer_overlay_address", peer.addr, "peer_underlay_address", mopAddress.Underlay)
			remove(peer)
			return
		case errors.Is(err, errOverlayMismatch):
			k.logger.Debug("overlay mismatch has occurred", "peer_overlay_address", peer.addr, "peer_underlay_address", mopAddress.Underlay)
			remove(peer)
			return
		case errors.Is(err, p2p.ErrPeerBlocklisted):
			k.logger.Debug("peer still in blocklist", "peer_address", mopAddress)
			k.logger.Warning("peer still in blocklist")
			return
		case err != nil:
			k.logger.Debug("peer not reachable from kademlia", "peer_address", mopAddress, "error", err)
	...
	for {
		...
		case <-k.manageC:
			...
			if k.connectedPeers.Length() == 0 {
				...
				if err := k.connectedPeers.EachBin(func(addr cluster.Address, _ uint8) (bool, bool, error) {
...
func (k *Kad) recordPeerLatencies(ctx context.Context) {
	...
	_ = k.connectedPeers.EachBin(func(addr cluster.Address, _ uint8) (bool, bool, error) {
		select {
		case <-ctx.Done():
			return false, false, nil
		case <-k.halt:
...
func (k *Kad) pruneOversaturatedBins(depth uint8) {
	...
	for i := range k.commonBinPrefixes {
		...
		for j := 0; peersToRemove > 0 && j < len(k.commonBinPrefixes[i]); j++ {
			...
			var newestPeer cluster.Address
...
func (k *Kad) balancedSlotPeers(pseudoAddr cluster.Address, peers []cluster.Address, po int) []cluster.Address {

	var ret []cluster.Address

	for _, peer := range peers {
		peerPo := cluster.ExtendedProximity(peer.Bytes(), pseudoAddr.Bytes())
...
func (k *Kad) Start(_ context.Context) error {
	...
	go func() {
		...
		err := k.addressBook.IterateOverlays(func(addr cluster.Address) (stop bool, err error) {
		...
		if err != nil {
			k.logger.Error(err, "address iterate overlays failed")
			return
		}
		k.AddPeers(addresses...)
...
func (k *Kad) previouslyConnected() []cluster.Address {
	loggerV1 := k.logger.V(1).Register()
	...
	var peers []cluster.Address

	for addr, p := range ss {
		if p.ConnectionTotalDuration > 0 {
			peers = append(peers, cluster.NewAddress([]byte(addr)))
		}
	}
...
func binSaturated(oversaturationAmount int, staticNode staticPeerFunc, k *Kad) binSaturationFunc {
	return func(bin uint8, peers, connected *pslice.PSlice, filter peerFilterFunc) bool {
		size := 0
		_ = connected.EachBin(func(addr cluster.Address, po uint8) (bool, bool, error) {
...
func (k *Kad) recalcDepth() {
	...
	_ = peers.EachBinRev(func(addr cluster.Address, bin uint8) (bool, bool, error) {
		if filter(addr) {
			return false, false, nil
		}
	...
	_ = peers.EachBin(func(addr cluster.Address, po uint8) (bool, bool, error) {
		if filter(addr) {
			return false, false, nil
		}
		peersCtr++
...
func (k *Kad) connect(ctx context.Context, peer cluster.Address, ma ma.Multiaddr) error {
	...
	case err != nil:
		...
		if (k.connectedPeers.Length() > 0 && quickPrune) || failedAttempts >= maxConnAttempts {
			...
			if err := k.addressBook.Remove(peer); err != nil {
				k.logger.Debug("could not remove peer from address", "peer_address", peer)
			}
...
func (k *Kad) Announce(ctx context.Context, peer cluster.Address, fullnode bool) error {
	var addrs []cluster.Address

outer:
	for bin := uint8(0); bin < cluster.MaxBins; bin++ {
		...
		for _, connectedPeer := range connectedPeers {
			...
			go func(connectedPeer cluster.Address) {

				// Create a new deadline mctx to prevent goroutine pile up
...
func (k *Kad) AnnounceTo(ctx context.Context, addressee, peer cluster.Address, fullnode bool) error {
	if !fullnode {
...
// be made to the peer.
func (k *Kad) AddPeers(addrs ...cluster.Address) {
...
func (k *Kad) Pick(peer p2p.Peer) bool {
	...
	po := cluster.Proximity(k.base.Bytes(), peer.Address.Bytes())
...
func (k *Kad) binReachablePeers(bin uint8) (peers []cluster.Address) {

	_ = k.EachPeerRev(func(p cluster.Address, po uint8) (bool, bool, error) {
...
func isStaticPeer(staticNodes []cluster.Address) func(overlay cluster.Address) bool {
	return func(overlay cluster.Address) bool {
...
func (k *Kad) Connected(ctx context.Context, peer p2p.Peer, forceConnection bool) (err error) {
	...
	po := cluster.Proximity(k.base.Bytes(), address.Bytes())
...
func (k *Kad) onConnected(ctx context.Context, addr cluster.Address) error {
...
func closestPeer(peers *pslice.PSlice, addr cluster.Address, spf sanctionedPeerFunc) (cluster.Address, error) {
	closest := cluster.ZeroAddress
...
func nClosePeerInSlice(peers []cluster.Address, addr cluster.Address, spf sanctionedPeerFunc, minPO uint8) (cluster.Address, bool) {
	for _, peer := range peers {
		if spf(peer) {
			continue
		}

		if cluster.ExtendedProximity(peer.Bytes(), addr.Bytes()) >= minPO {
			return peer, true
		}
	}
...
func closestPeerFunc(closest *cluster.Address, addr cluster.Address, spf sanctionedPeerFunc) func(peer cluster.Address, po uint8) (bool, bool, error) {
	return func(peer cluster.Address, po uint8) (bool, bool, error) {
...
func (k *Kad) ClosestPeer(addr cluster.Address, includeSelf bool, filter topology.Filter, skipPeers ...cluster.Address) (cluster.Address, error) {
	if k.connectedPeers.Length() == 0 {
	...
	err := k.EachPeerRev(func(peer cluster.Address, po uint8) (bool, bool, error) {
	...
	if closest.IsZero() { // no peers
		return cluster.Address{}, topology.ErrNotFound // only for light nodes
	}

	// check if self
	if closest.Equal(k.base) {
		return cluster.Address{}, topology.ErrWantSelf
	}
...
func (k *Kad) IsWithinDepth(addr cluster.Address) bool {
	return cluster.Proximity(k.base.Bytes(), addr.Bytes()) >= k.NeighborhoodDepth()
}
...
func (k *Kad) EachNeighbor(f topology.EachPeerFunc) error {
	depth := k.NeighborhoodDepth()
	fn := func(a cluster.Address, po uint8) (bool, bool, error) {
		if po < depth {
			return true, false, nil
		}
		return f(a, po)
...
func (k *Kad) EachPeer(f topology.EachPeerFunc, filter topology.Filter) error {
	return k.connectedPeers.EachBin(func(addr cluster.Address, po uint8) (bool, bool, error) {
...
func (k *Kad) EachPeerRev(f topology.EachPeerFunc, filter topology.Filter) error {
	return k.connectedPeers.EachBinRev(func(addr cluster.Address, po uint8) (bool, bool, error) {
		if filter.Reachable && k.peerFilter(addr) {
			return false, false, nil
		}
		return f(addr, po)
...
func (k *Kad) Reachable(addr cluster.Address, status p2p.ReachabilityStatus) {
	loggerV1 := k.logger.V(1).Register()
...
func (k *Kad) IsBalanced(bin uint8) bool {
	...
	for i := range k.commonBinPrefixes[bin] {
		...
		closestConnectedPO := cluster.ExtendedProximity(closestConnectedPeer.Bytes(), pseudoAddr.Bytes())
...
func (k *Kad) Snapshot() *topology.KadParams {
	...
	for i := int(cluster.MaxPO); i >= 0; i-- {
		infos = append(infos, topology.BinInfo{})
	}

	ss := k.collector.Snapshot(time.Now())

	_ = k.connectedPeers.EachBin(func(addr cluster.Address, po uint8) (bool, bool, error) {
	...
	_ = k.knownPeers.EachBin(func(addr cluster.Address, po uint8) (bool, bool, error) {
...
func randomSubset(addrs []cluster.Address, count int) ([]cluster.Address, error) {
	if count >= len(addrs) {
		return addrs, nil
	}
...
func (k *Kad) randomPeer(bin uint8) (cluster.Address, error) {
	...
	if len(peers) == 0 {
		return cluster.ZeroAddress, errEmptyBin
	}
