...
func TestHandshake(t *testing.T) {
	...
	t.Run("Handshake - OK", func(t *testing.T) {
		...
		if err := w.WriteMsg(&pb.SynAck{
			...
			Ack: &pb.Ack{HopAddress: node1HopAddress},
		}); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("Handshake - invalid ack", func(t *testing.T) {
		...
		if err := w.WriteMsg(&pb.SynAck{
			...
			Ack: &pb.Ack{HopAddress: node2HopAddress},
		}); err != nil {
			t.Fatal(err)
		}
		...
		if err := w.WriteMsg(&pb.SynAck{
			Syn: &pb.Syn{
				HopAddress: &pb.HopAddress{
					Underlay:  node2HopAddress.Underlay,
					Signature: []byte("wrong signature"),
					Overlay:   node2HopAddress.Overlay,
				},
				NetworkID: node2Info.NetworkID,
				Light:     node2Info.Light,
			},
	...
	t.Run("Handle - OK", func(t *testing.T) {
		...
		if err := w.WriteMsg(&pb.Ack{HopAddress: node1HopAddress}); err != nil {
			t.Fatal(err)
		}
		...
		testInfo(t, node1Info, handshake.Info{
			Overlay:   swarm.NewAddress(got.Syn.HopAddress.Overlay),
			Underlay:  got.Syn.HopAddress.Underlay,
	...
	t.Run("Handle - invalid ack", func(t *testing.T) {
		...
		if err := w.WriteMsg(&pb.Ack{HopAddress: node2HopAddress}); err != nil {
			t.Fatal(err)
		}
