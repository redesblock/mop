...
type Syn struct {
	HopAddress *HopAddress `protobuf:"bytes,1,opt,name=HopAddress,proto3" json:"HopAddress,omitempty"`
...
func (m *Syn) GetHopAddress() *HopAddress {
	if m != nil {
		return m.HopAddress
	}
	return nil
...
func (m *Ack) GetHopAddress() *HopAddress {
	if m != nil {
		return m.HopAddress
	}
	return nil
...
func init() {
	...
	proto.RegisterType((*HopAddress)(nil), "handshake.HopAddress")
}
...
func (m *Syn) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if m.HopAddress != nil {
		{
			size, err := m.HopAddress.MarshalToSizedBuffer(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
...
func (m *Syn) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field HopAddress", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.HopAddress == nil {
				m.HopAddress = &HopAddress{}
			}
			if err := m.HopAddress.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 2:
