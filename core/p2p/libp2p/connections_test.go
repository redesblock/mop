...
import (
	...
	libswarmtesting "github.com/libp2p/go-libp2p-swarm/testing"
...
func TestConnectDisconnect(t *testing.T) {
	...
	mopAddress, err := s2.Connect(ctx, addr)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := s2.Disconnect(mopAddress.Overlay, testDisconnectMsg); err != nil {
		t.Fatal(err)
	}
...
func TestDoubleDisconnect(t *testing.T) {
	...
	if err := s2.Disconnect(mopAddress.Overlay, testDisconnectMsg); !errors.Is(err, p2p.ErrPeerNotFound) {
...
func TestTopologyNotifier(t *testing.T) {
	var (
		...
		ab1, ab2 = address.New(mock.NewStateStore()), address.New(mock.NewStateStore())
	...
	mopAddr2, err := s1.Connect(ctx, addr2)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := s1.Disconnect(mopAddr2.Overlay, testDisconnectMsg); err != nil {
		t.Fatal(err)
	}
	expectPeers(t, s1)
...
func TestTopologyAnnounce(t *testing.T) {
	var (
		...
		ab1, ab2, ab3 = address.New(mock.NewStateStore()), address.New(mock.NewStateStore()), address.New(mock.NewStateStore())

		announceCalled   = false
		announceToCalled = false

		n1a = func(context.Context, cluster.Address, bool) error {
			mtx.Lock()
			announceCalled = true
			mtx.Unlock()
			return nil
		}
		n1at = func(context.Context, cluster.Address, cluster.Address, bool) error {
			mtx.Lock()
	...
	s1, _ := newService(t, 1, libp2pServiceOpts{
		libp2pOpts: libp2p.WithHostFactory(
			func(_ ...libp2pm.Option) (host.Host, error) {
				return bhost.NewHost(libswarmtesting.GenSwarm(t), &bhost.HostOpts{})
			},
		),
	})
	defer s1.Close()
...
func expectZeroAddress(t *testing.T, addrs ...cluster.Address) {
	t.Helper()
	for i, a := range addrs {
...
func waitAddrSet(t *testing.T, addr *cluster.Address, mtx *sync.Mutex, exp cluster.Address) {
	t.Helper()
	for i := 0; i < 20; i++ {
		mtx.Lock()
		if addr.Equal(exp) {
...
func checkAddressbook(t *testing.T, ab address.Getter, overlay cluster.Address, underlay ma.Multiaddr) {
	t.Helper()
...
func (n *notifiee) Announce(ctx context.Context, a cluster.Address, full bool) error {
	return n.announce(ctx, a, full)
}

func (n *notifiee) AnnounceTo(ctx context.Context, a, b cluster.Address, full bool) error {
...
func (n *notifiee) Reachable(addr cluster.Address, status p2p.ReachabilityStatus) {
...
type (
	...
	dFunc            func(p2p.Peer)
	announceFunc     func(context.Context, cluster.Address, bool) error
	announceToFunc   func(context.Context, cluster.Address, cluster.Address, bool) error
...
var noopAnnounce = func(context.Context, cluster.Address, bool) error { return nil }
var noopAnnounceTo = func(context.Context, cluster.Address, cluster.Address, bool) error { return nil }
var noopReachability = func(p2p.ReachabilityStatus) {}
var noopReachable = func(cluster.Address, p2p.ReachabilityStatus) {}
