...
func TestTracing(t *testing.T) {
	tracer1, closer1, err := tracer.NewTracer(&tracer.Options{
		Enabled:     true,
	...
	tracer2, closer2, err := tracer.NewTracer(&tracer.Options{
		Enabled:     true,
		ServiceName: "mop-test",
	})
	if err != nil {
	...
	if fmt.Sprint(span.Context()) == "" {
		t.Error("not tracer span context to send")
	}
	...
	if handledTracingSpan == "" {
		t.Error("got not tracer span context in handler")
	}
}
