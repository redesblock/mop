...
func TestRecorder(t *testing.T) {
	...
	ask := func(ctx context.Context, s p2p.Streamer, address cluster.Address, questions ...string) (answers []string, err error) {
	...
	aa, err := ask(context.Background(), recorder, cluster.ZeroAddress, questions...)
	if err != nil {
		t.Fatal(err)
	}
	...
	records, err := recorder.Records(cluster.ZeroAddress, testProtocolName, testProtocolVersion, testStreamName)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRecorder_fullcloseWithRemoteClose(t *testing.T) {
	...
	request := func(ctx context.Context, s p2p.Streamer, address cluster.Address) (err error) {
		stream, err := s.NewStream(ctx, address, nil, testProtocolName, testProtocolVersion, testStreamName)
		if err != nil {
...
func TestRecorder_withMiddlewares(t *testing.T) {
	...
	request := func(ctx context.Context, s p2p.Streamer, address cluster.Address) error {
		stream, err := s.NewStream(ctx, address, nil, testProtocolName, testProtocolVersion, testStreamName)
		if err != nil {
			return fmt.Errorf("new stream: %w", err)
		}
...
func TestRecorder_withStreamError(t *testing.T) {
	...
	recorder := streamtest.New(
		...
		streamtest.WithStreamError(func(addr cluster.Address, _, _, _ string) error {
