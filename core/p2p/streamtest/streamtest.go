...
type Recorder struct {
	...
	streamErr          func(cluster.Address, string, string, string) error
...
func WithBaseAddr(a cluster.Address) Option {
	return optionFunc(func(r *Recorder) {
		r.base = a
	})
}
...
func WithStreamError(streamErr func(cluster.Address, string, string, string) error) Option {
	return optionFunc(func(r *Recorder) {
...
func (r *Recorder) NewStream(ctx context.Context, addr cluster.Address, h p2p.Headers, protocolName, protocolVersion, streamName string) (p2p.Stream, error) {
	if r.streamErr != nil {
...
func (r *RecorderDisconnecter) Disconnect(overlay cluster.Address, _ string) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *RecorderDisconnecter) Blocklist(overlay cluster.Address, d time.Duration, _ string) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *RecorderDisconnecter) IsDisconnected(overlay cluster.Address) bool {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *RecorderDisconnecter) IsBlocklisted(overlay cluster.Address) (bool, time.Duration) {
	r.mu.RLock()
	defer r.mu.RUnlock()
