...
func selectAccount(clef ExternalSignerInterface, ethAddress *common.Address) (accounts.Account, error) {
	// get the list of available BNB Smart Chain accounts
...
// SignData signs with the text/plain type which is the standard BNB Smart Chain prefix method.
func (c *clefSigner) Sign(data []byte) ([]byte, error) {
	return c.clef.SignData(c.account, accounts.MimetypeTextPlain, data)
}

// SignTx signs an BNB Smart Chain transaction.
...
// BSCAddress returns the BNB Smart Chain this signer uses.
func (c *clefSigner) BSCAddress() (common.Address, error) {
