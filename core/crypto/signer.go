...
type Signer interface {
	...
	BSCAddress() (common.Address, error)
}
...
// BSCAddress returns the BNB Smart Chain this signer uses.
func (d *defaultSigner) BSCAddress() (common.Address, error) {
	publicKey, err := d.PublicKey()
	if err != nil {
		return common.Address{}, err
	}
	eth, err := NewBSCAddress(*publicKey)
	if err != nil {
		return common.Address{}, err
	}
...
func (d *defaultSigner) sign(sighash []byte, isCompressedKey bool) ([]byte, error) {
	...
	// Convert to BNB Smart Chain signature format with 'recovery id' v at the end.
	v := signature[0]
