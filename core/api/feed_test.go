...
var expReference = cluster.MustParseHexAddress("7ddba94d59de5cc57e89a69185ba26959f42511710235d1ce95930c1e1c617a8")
...
func TestFeed_Get(t *testing.T) {
	...
	t.Run("with at", func(t *testing.T) {
		...
		h := respHeaders[api.ClusterFeedIndexHeader]
		if len(h) == 0 {
			t.Fatal("expected cluster feed index header to be set")
		}
	...
	t.Run("latest", func(t *testing.T) {
		...
		if h := respHeaders[api.ClusterFeedIndexHeader]; len(h) > 0 {
			b, err := hex.DecodeString(h[0])
			if err != nil {
				t.Fatal(err)
			}
...
func newMockLookup(at, after int64, ch cluster.Chunk, err error, cur, next feeds.Index) *mockLookup {
	return &mockLookup{at: at, after: after, chunk: ch, err: err, cur: cur, next: next}
}

func (l *mockLookup) At(_ context.Context, at, after int64) (cluster.Chunk, feeds.Index, feeds.Index, error) {
	if l.at == -1 {
...
func toChunk(t *testing.T, at uint64, payload []byte) cluster.Chunk {
	ts := make([]byte, 8)
