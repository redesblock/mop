...
func TestPssWebsocketSingleHandler(t *testing.T) {
	...
	cl.SetReadLimit(flock.ChunkSize)
...
func TestPssSend(t *testing.T) {
	var (
		...
		sendFn = func(ctx context.Context, targets pss.Targets, chunk flock.Chunk) error {
			mtx.Lock()
...
type pssSendFn func(context.Context, pss.Targets, flock.Chunk) error
type mpss struct {
	f pssSendFn
}
...
func (m *mpss) Send(ctx context.Context, topic pss.Topic, payload []byte, _ postage.Voucher, recipient *ecdsa.PublicKey, targets pss.Targets) error {
...
func (m *mpss) TryUnwrap(_ flock.Chunk) {
	panic("not implemented") // TODO: Implement
}
