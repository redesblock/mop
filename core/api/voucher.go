...
func (s *Service) voucherAccessHandler(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !s.voucherSem.TryAcquire(1) {
			s.logger.Debug("voucher access: simultaneous on-chain operations not supported")
			s.logger.Error(nil, "voucher access: simultaneous on-chain operations not supported")
			jsonhttp.TooManyRequests(w, "simultaneous on-chain operations not supported")
			return
		}
		defer s.voucherSem.Release(1)

		h.ServeHTTP(w, r)
	})
}

func (s *Service) voucherSyncStatusCheckHandler(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		done, err := s.syncStatus()
		if err != nil {
			s.logger.Debug("voucher access: syncing failed", "error", err)
			s.logger.Error(nil, "voucher access: syncing failed")
			jsonhttp.ServiceUnavailable(w, "voucher: syncing failed")
			return
		}
		if !done {
			s.logger.Debug("voucher access: syncing in progress")
			s.logger.Error(nil, "voucher access: syncing in progress")
...
func (s *Service) voucherCreateHandler(w http.ResponseWriter, r *http.Request) {
	...
	batchID, err := s.voucherContract.CreateBatch(ctx, amount, uint8(depth), immutable, label)
	if err != nil {
	...
	jsonhttp.Created(w, &voucherCreateResponse{
		BatchID: batchID,
	})
}

type voucherStampResponse struct {
	BatchID       hexByte        `json:"batchID"`
...
type voucherStampsResponse struct {
	Stamps []voucherStampResponse `json:"stamps"`
}
...
type voucherStampBucketsResponse struct {
	Depth            uint8        `json:"depth"`
	BucketDepth      uint8        `json:"bucketDepth"`
...
func (s *Service) voucherGetStampsHandler(w http.ResponseWriter, r *http.Request) {
	isAll := strings.ToLower(r.URL.Query().Get("all")) == "true"
	resp := voucherStampsResponse{}
	resp.Stamps = make([]voucherStampResponse, 0, len(s.post.StampIssuers()))
	for _, v := range s.post.StampIssuers() {
		...
		if isAll || exists {
			resp.Stamps = append(resp.Stamps, voucherStampResponse{
				BatchID:       v.ID(),
...
func (s *Service) voucherGetAllStampsHandler(w http.ResponseWriter, _ *http.Request) {
	batches := make([]voucherBatchResponse, 0)
	err := s.batchStore.Iterate(func(b *voucher.Batch) (bool, error) {
		...
		batches = append(batches, voucherBatchResponse{
			BatchID:       b.ID,
	...
	batchesRes := struct {
		Batches []voucherBatchResponse `json:"batches"`
	}{
		Batches: batches,
	}
...
func (s *Service) voucherGetStampBucketsHandler(w http.ResponseWriter, r *http.Request) {
	idStr := mux.Vars(r)["id"]
	if len(idStr) != 64 {
	...
	b := issuer.Buckets()
	resp := voucherStampBucketsResponse{
		Depth:            issuer.Depth(),
...
func (s *Service) voucherGetStampHandler(w http.ResponseWriter, r *http.Request) {
	idStr := mux.Vars(r)["id"]
	if len(idStr) != 64 {
		s.logger.Error(nil, "get stamp issuer: invalid batch id string length", "string", idStr, "length", len(idStr))
		jsonhttp.BadRequest(w, "invalid batchID")
...
func (s *Service) voucherTopUpHandler(w http.ResponseWriter, r *http.Request) {
	idStr := mux.Vars(r)["id"]
	if len(idStr) != 64 {
	...
	err = s.voucherContract.TopUpBatch(ctx, id, amount)
	if err != nil {
		if errors.Is(err, vouchercontract.ErrInsufficientFunds) {
	...
	jsonhttp.Accepted(w, &voucherCreateResponse{
		BatchID: id,
	})
}

func (s *Service) voucherDiluteHandler(w http.ResponseWriter, r *http.Request) {
	idStr := mux.Vars(r)["id"]
	if len(idStr) != 64 {
	...
	err = s.voucherContract.DiluteBatch(ctx, id, uint8(depth))
	if err != nil {
		if errors.Is(err, vouchercontract.ErrInvalidDepth) {
