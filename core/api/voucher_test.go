...
import (
	...
	contractMock "github.com/redesblock/mop/core/incentives/voucher/vouchercontract/mock"
	"github.com/redesblock/mop/core/mctx"
	"github.com/redesblock/mop/core/util/bigint"
)

func TestVoucherCreateStamp(t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		...
		jsonhttptest.Request(t, ts, http.MethodPost, createBatch(initialBalance, depth, label), http.StatusCreated,
			jsonhttptest.WithExpectedJSONResponse(&api.VoucherCreateResponse{
				BatchID: batchID,
			}),
		)
	})
	t.Run("with-custom-gas", func(t *testing.T) {
		contract := contractMock.New(
			contractMock.WithCreateBatchFunc(func(ctx context.Context, ib *big.Int, d uint8, i bool, l string) ([]byte, error) {
				...
				if mctx.GetGasPrice(ctx).Cmp(big.NewInt(10000)) != 0 {
					return nil, fmt.Errorf("called with wrong gas price. wanted %d, got %d", 10000, mctx.GetGasPrice(ctx))
				}
				return batchID, nil
			}),
		)
	...
	t.Run("syncing failed", func(t *testing.T) {
		...
		jsonhttptest.Request(t, ts, http.MethodPost, createBatch(initialBalance, depth, label), http.StatusServiceUnavailable,
			jsonhttptest.WithExpectedJSONResponse(&jsonhttp.StatusResponse{
				Message: "voucher: syncing failed",
				Code:    503,
			}),
		)
	})
...
func TestVoucherGetStamps(t *testing.T) {
	b := vouchertesting.MustNewBatch(vouchertesting.WithValue(20))
	...
	t.Run("single stamp", func(t *testing.T) {
		...
		jsonhttptest.Request(t, ts, http.MethodGet, "/stamps", http.StatusOK,
			jsonhttptest.WithExpectedJSONResponse(&api.VoucherStampsResponse{
				Stamps: []api.VoucherStampResponse{
					{
						BatchID:       b.ID,
	...
	t.Run("expired batch", func(t *testing.T) {
		...
		jsonhttptest.Request(t, tsForNonExistingBatch, http.MethodGet, "/stamps", http.StatusOK,
			jsonhttptest.WithExpectedJSONResponse(&api.VoucherStampsResponse{Stamps: []api.VoucherStampResponse{}}),
		)
	...
	t.Run("expired Stamp", func(t *testing.T) {
		eb := vouchertesting.MustNewBatch(vouchertesting.WithValue(20))

		esi := voucher.NewStampIssuer("", "", eb.ID, big.NewInt(3), 11, 10, 1000, true)
		emp := mockpost.New(mockpost.WithIssuer(esi))
		emp.HandleStampExpiry(eb.ID)
		ecs := &voucher.ChainState{Block: 10, TotalAmount: big.NewInt(15), CurrentPrice: big.NewInt(12)}
	...
	t.Run("single expired Stamp", func(t *testing.T) {
		...
		jsonhttptest.Request(t, ts, http.MethodGet, "/stamps/"+hex.EncodeToString(eb.ID), http.StatusOK,
			jsonhttptest.WithExpectedJSONResponse(&api.VoucherStampResponse{
				BatchID:       eb.ID,
				Utilization:   esi.Utilization(),
				Usable:        false,
				Label:         esi.Label(),
...
func TestGetAllBatches(t *testing.T) {
	...
	oneBatch := struct {
		Batches []api.VoucherBatchResponse `json:"batches"`
	}{
		Batches: []api.VoucherBatchResponse{
			{
				BatchID:       b.ID,
...
func TestVoucherGetStamp(t *testing.T) {
	b := vouchertesting.MustNewBatch()
	b.Value = big.NewInt(20)
	si := voucher.NewStampIssuer("", "", b.ID, big.NewInt(3), 11, 10, 1000, true)
	mp := mockpost.New(mockpost.WithIssuer(si))
...
func TestVoucherGetBuckets(t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		jsonhttptest.Request(t, ts, http.MethodGet, "/stamps/"+batchOkStr+"/buckets", http.StatusOK,
			jsonhttptest.WithExpectedJSONResponse(&api.VoucherStampBucketsResponse{
				Depth:            si.Depth(),
				BucketDepth:      si.BucketDepth(),
...
func TestVoucherTopUpStamp(t *testing.T) {
	...
	t.Run("with-custom-gas", func(t *testing.T) {
		contract := contractMock.New(
			contractMock.WithTopUpBatchFunc(func(ctx context.Context, id []byte, ib *big.Int) error {
				...
				if mctx.GetGasPrice(ctx).Cmp(big.NewInt(10000)) != 0 {
					return fmt.Errorf("called with wrong gas price. wanted %d, got %d", 10000, mctx.GetGasPrice(ctx))
				}
				return nil
			}),
		)
...
func TestVoucherDiluteStamp(t *testing.T) {
...
// Tests the voucherAccessHandler middleware for any set of operations that are guarded
// by the voucher semaphore
func TestVoucherAccessHandler(t *testing.T) {
	...
	success := []operation{
		{
			...
			resp: &api.VoucherCreateResponse{
				BatchID: batchOk,
			},
		},
		{
