...
func TestMopFiles(t *testing.T) {
	var (
		fileUploadResource   = "/mop"
		fileDownloadResource = func(addr string) string { return "/mop/" + addr }
	...
	t.Run("tar-file-upload", func(t *testing.T) {
		...
		address := cluster.MustParseHexAddress("f30c0aa7e9e2a0ef4c9b1b750ebfeaeb7c7c24da700bb089da19a46e3677824b")
	...
	t.Run("tar-file-upload-with-pins", func(t *testing.T) {
		tr := tarFiles(t, []f{
			{
				data: []byte("robots text"),
				name: "robots.txt",
		...
		reference := cluster.MustParseHexAddress("f30c0aa7e9e2a0ef4c9b1b750ebfeaeb7c7c24da700bb089da19a46e3677824b")
		rcvdHeader := jsonhttptest.Request(t, client, http.MethodPost, fileUploadResource, http.StatusCreated,
			jsonhttptest.WithRequestHeader(api.ClusterDeferredUploadHeader, "true"),
			jsonhttptest.WithRequestHeader(api.ClusterVoucherBatchIdHeader, batchOkStr),
	...
	t.Run("encrypt-decrypt", func(t *testing.T) {
		...
		if rcvdHeader.Get(api.ContentTypeHeader) != "image/jpeg; charset=utf-8" {
	...
	t.Run("upload-then-download-and-check-data", func(t *testing.T) {
		...
		if rcvdHeader.Get(api.ContentTypeHeader) != "text/html; charset=utf-8" {
			t.Fatal("Invalid content type detected")
		}
...
func TestMopFilesRangeRequests(t *testing.T) {
	...
	for _, upload := range uploads {
		t.Run(upload.name, func(t *testing.T) {
			...
			if upload.name == "dir" {
				testOpts = append(testOpts, jsonhttptest.WithRequestHeader(api.ClusterCollectionHeader, "True"))
			}
...
func TestFeedIndirection(t *testing.T) {
	...
	var (
		updateData      = []byte("<h1>Cluster Feeds Hello World!</h1>")
		mockStatestore  = statestore.NewStateStore()
		logger          = log.Noop
		storer          = smock.NewStorer()
		client, _, _, _ = newTestServer(t, testServerOptions{
	...
	options := []jsonhttptest.Option{
		...
		jsonhttptest.WithUnmarshalJSONResponse(&resp),
		jsonhttptest.WithRequestHeader(api.ClusterIndexDocumentHeader, "index.html"),
	}

	// verify directory tar upload response
	jsonhttptest.Request(t, client, http.MethodPost, "/mop", http.StatusCreated, options...)
	...
	// called from the mop endpoint. then call the mop endpoint with
	...
	err = m.Add(ctx, manifest.RootPath, manifest.NewEntry(cluster.NewAddress(emptyAddr), map[string]string{
	...
	jsonhttptest.Request(t, client, http.MethodGet, mopDownloadResource(manifRef.String(), ""), http.StatusOK,
