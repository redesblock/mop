...
func TestTags(t *testing.T) {
	...
	t.Run("create tag with invalid id", func(t *testing.T) {
		jsonhttptest.Request(t, client, http.MethodPost, chunksResource, http.StatusBadRequest,
			...
			jsonhttptest.WithRequestHeader(api.ClusterTagHeader, "invalid_id.jpg"), // the value should be uint32
		)
	})
	...
	t.Run("create tag upload chunk", func(t *testing.T) {
		...
		rcvdHeaders := jsonhttptest.Request(t, client, http.MethodPost, chunksResource, http.StatusCreated,
			...
			jsonhttptest.WithRequestBody(bytes.NewReader(chunk.Data())),
			jsonhttptest.WithExpectedJSONResponse(api.ChunkAddressResponse{Reference: chunk.Address()}),
			jsonhttptest.WithRequestHeader(api.ClusterTagHeader, strconv.FormatUint(uint64(tr.Uid), 10)),
		)

		isTagFoundInResponse(t, rcvdHeaders, &tr)
		tagValueTest(t, tr.Uid, 1, 1, 1, 0, 0, 0, cluster.ZeroAddress, client)
	})
	...
	t.Run("create tag upload chunk stream", func(t *testing.T) {
		...
		wsHeaders.Set(api.ClusterTagHeader, strconv.FormatUint(uint64(tr.Uid), 10))
		...
		tagValueTest(t, tr.Uid, 5, 5, 0, 0, 0, 0, cluster.ZeroAddress, client)
	})
	...
	t.Run("done split", func(t *testing.T) {
		...
		jsonhttptest.Request(t, client, http.MethodPost, chunksResource, http.StatusCreated,
			...
			jsonhttptest.WithRequestHeader(api.ClusterVoucherBatchIdHeader, batchOkStr),
			jsonhttptest.WithRequestBody(bytes.NewReader(chunk.Data())),
			jsonhttptest.WithRequestHeader(api.ClusterTagHeader, fmt.Sprint(tagId)),
		)
	...
	t.Run("file tags", func(t *testing.T) {
		...
		expectedHash := cluster.MustParseHexAddress("40e739ebdfd18292925bba4138cd097db9aa18c1b57e74042f48469b48da33a8")
		...
		tagId, err := strconv.Atoi(respHeaders.Get(api.ClusterTagHeader))
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("dir tags", func(t *testing.T) {
		...
		expectedHash := cluster.MustParseHexAddress("42bc27c9137c93705ffbc2945fa1aab0e8e1826f1500b7f06f6e3f86f617213b")
		expectedResponse := api.MopUploadResponse{Reference: expectedHash}
	...
	t.Run("bytes tags", func(t *testing.T) {
		...
		sentHeaders.Set(api.ClusterTagHeader, strconv.FormatUint(uint64(tr.Uid), 10))

		g := mockbytes.New(0, mockbytes.MockTypeStandard).WithModulus(255)
		dataChunk, err := g.SequentialBytes(cluster.ChunkSize)
		if err != nil {
			t.Fatal(err)
		}

		rootAddress := cluster.MustParseHexAddress("5e2a21902f51438be1adbd0e29e1bd34c53a21d3120aefa3c7275129f2f88de9")

		content := make([]byte, cluster.ChunkSize*2)
		copy(content[cluster.ChunkSize:], dataChunk)
		copy(content[:cluster.ChunkSize], dataChunk)
		...
		tagValueTest(t, id, 3, 3, 1, 0, 0, 3, cluster.ZeroAddress, client)
	})
}
...
func isTagFoundInResponse(t *testing.T, headers http.Header, tr *api.TagResponse) uint32 {
	...
	idStr := headers.Get(api.ClusterTagHeader)
	if idStr == "" {
...
func tagValueTest(t *testing.T, id uint32, split, stored, seen, sent, synced, total int64, address cluster.Address, client *http.Client) {
	t.Helper()
	tag := api.TagResponse{}
