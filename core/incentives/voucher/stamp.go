...
var _ cluster.Stamp = (*Stamp)(nil)

// Stamp represents a voucher stamp as attached to a chunk.
type Stamp struct {
...
type ValidStampFn func(chunk cluster.Chunk, stampBytes []byte) (cluster.Chunk, error)
...
func ValidStamp(batchStore Storer) ValidStampFn {
	return func(chunk cluster.Chunk, stampBytes []byte) (cluster.Chunk, error) {
		stamp := new(Stamp)
...
// Valid checks the validity of the voucher stamp; in particular:
...
func (s *Stamp) Valid(chunkAddr cluster.Address, ownerAddr []byte, depth, bucketDepth uint8, immutable bool) error {
	...
	signerAddr, err := crypto.NewBSCAddress(*signerPubkey)
	if err != nil {
		return err
	}
