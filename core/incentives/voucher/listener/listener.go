...
const (
	blockPage          = 5000      // how many blocks to chainsync every time we page
	tailSize           = 4         // how many blocks to tail from the tip of the chain
	defaultBatchFactor = uint64(5) // // minimal number of blocks to chainsync at once
)
...
var (
	...
	// batchCreatedTopic is the voucher contract's batch created event topic
	batchCreatedTopic = voucherStampABI.Events["BatchCreated"].ID
	// batchTopupTopic is the voucher contract's batch topup event topic
	batchTopupTopic = voucherStampABI.Events["BatchTopUp"].ID
	// batchDepthIncreaseTopic is the voucher contract's batch dilution event topic
	batchDepthIncreaseTopic = voucherStampABI.Events["BatchDepthIncrease"].ID
	// priceUpdateTopic is the voucher contract's price update event topic
	priceUpdateTopic = voucherStampABI.Events["PriceUpdate"].ID

	ErrVoucherSyncingStalled = errors.New("voucher syncing stalled")
)
...
type listener struct {
	...
	voucherStampAddress common.Address
	quit                chan struct{}
	wg                  sync.WaitGroup
	metrics             metrics
...
func New(
	...
	return &listener{
		...
		ev:                  ev,
		blockTime:           blockTime,
		voucherStampAddress: voucherStampAddress,
...
func (l *listener) processEvent(e types.Log, updater voucher.EventUpdater) error {
	defer l.metrics.EventsProcessed.Inc()
	switch e.Topics[0] {
	case batchCreatedTopic:
		c := &batchCreatedEvent{}
		err := transaction.ParseEvent(&voucherStampABI, "BatchCreated", c, e)
		if err != nil {
			return err
		}
	...
	case batchTopupTopic:
		c := &batchTopUpEvent{}
		err := transaction.ParseEvent(&voucherStampABI, "BatchTopUp", c, e)
		if err != nil {
			return err
		}
	...
	case batchDepthIncreaseTopic:
		...
		err := transaction.ParseEvent(&voucherStampABI, "BatchDepthIncrease", c, e)
		if err != nil {
			return err
		}
	...
	case priceUpdateTopic:
		c := &priceUpdateEvent{}
		err := transaction.ParseEvent(&voucherStampABI, "PriceUpdate", c, e)
		if err != nil {
			return err
		}
...
func (l *listener) Listen(from uint64, updater voucher.EventUpdater, initState *voucher.ChainSnapshot) <-chan error {
	...
	listenf := func() error {
		...
		for {
			...
			if time.Since(lastProgress) >= l.stallingTimeout {
				return ErrVoucherSyncingStalled
			}
			...
			select {
			case <-paged:
				// if we paged then it means there's more things to chainsync on
			...
			// consider to-tailSize as the "latest" block we need to chainsync to
			to = to - tailSize
...
func (l *listener) Close() error {
	...
	case <-time.After(5 * time.Second):
		return errors.New("voucher listener closed with running goroutines")
	}
	return nil
}
