...
var voucherStampAddress common.Address = common.HexToAddress("eeee")
...
func TestListener(t *testing.T) {
	...
	t.Run("create event", func(t *testing.T) {
		...
		l := listener.New(nil, logger, mf, voucherStampAddress, 1, stallingTimeout, backoffTime)
		<-l.Listen(0, ev, nil)
	...
	t.Run("do not shutdown on error event", func(t *testing.T) {
		...
		l := listener.New(nil, logger, mf, voucherStampAddress, 1, stallingTimeout, 0*time.Second)
		<-l.Listen(0, ev, nil)
	...
	t.Run("shutdown on stalling", func(t *testing.T) {
		...
		c := util.NewSignaler()
		l := listener.New(c, logger, mf, voucherStampAddress, 1, 50*time.Millisecond, 0*time.Second)
		<-l.Listen(0, ev, nil)
	...
	t.Run("shutdown on processing error", func(t *testing.T) {
		...
		c := util.NewSignaler()
		l := listener.New(c, logger, mf, voucherStampAddress, 1, stallingTimeout, backoffTime)
		<-l.Listen(0, ev, nil)
...
func TestListenerBatchState(t *testing.T) {
	...
	snapshot := &voucher.ChainSnapshot{
		Events: []types.Log{
			create.toLog(496),
			topup.toLog(497),
...
func (u *updater) Start(uint64, *voucher.ChainSnapshot, chan struct{}) error { return nil }
...
func (c createArgs) toLog(blockNumber uint64) types.Log {
	b, err := listener.VoucherStampABI.Events["BatchCreated"].Inputs.NonIndexed().Pack(c.amount, c.normalisedAmount, common.BytesToAddress(c.owner), c.bucketDepth, c.depth, c.immutable)
	if err != nil {
		panic(err)
	}
	return types.Log{
...
func (ta topupArgs) toLog(blockNumber uint64) types.Log {
	b, err := listener.VoucherStampABI.Events["BatchTopUp"].Inputs.NonIndexed().Pack(ta.amount, ta.normalisedBalance)
	if err != nil {
		panic(err)
	}
	return types.Log{
...
func (d depthArgs) toLog(blockNumber uint64) types.Log {
	b, err := listener.VoucherStampABI.Events["BatchDepthIncrease"].Inputs.NonIndexed().Pack(d.depth, d.normalisedBalance)
	if err != nil {
		panic(err)
	}
	return types.Log{
...
func (p priceArgs) toLog(blockNumber uint64) types.Log {
	b, err := listener.VoucherStampABI.Events["PriceUpdate"].Inputs.NonIndexed().Pack(p.price)
	if err != nil {
		panic(err)
	}
	return types.Log{
