...
// Service is the voucher service interface.
...
// service handles voucher batches
...
func NewService(store storage.StateStorer, voucherStore Storer, chainID int64) (Service, error) {
	s := &service{
		store:        store,
...
func (ps *service) IssuerUsable(st *StampIssuer) bool {
	cs := ps.voucherStore.GetChainState()
...
func (ps *service) key() string {
	return fmt.Sprintf(voucherPrefix+"%d", ps.chainID)
}
...
func (ps *service) SetExpired() error {
	...
	for _, v := range ps.issuers {
		exists, err := ps.voucherStore.Exists(v.ID())
		if err != nil {
			return err
		}
		if !exists {
