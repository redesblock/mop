...
type batchService struct {
	...
	storer        voucher.Storer
	logger        log.Logger
	listener      voucher.Listener
	owner         []byte
...
var ErrInterruped = errors.New("voucher chainsync interrupted")

func (svc *batchService) Start(startBlock uint64, initState *voucher.ChainSnapshot, interrupt chan struct{}) (err error) {
	dirty := false
