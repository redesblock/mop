...
import (
	...
	mocks "github.com/redesblock/mop/core/storer/statestore/mock"
...
func (*mockListener) Listen(from uint64, updater voucher.EventUpdater, _ *voucher.ChainSnapshot) <-chan error {
	c := make(chan error, 1)
	c <- nil
	return c
}
...
func (m *mockBatchListener) HandleCreate(b *voucher.Batch, topUpAmount *big.Int) error {
	m.createCount++
	return nil
}
...
func TestBatchServiceCreate(t *testing.T) {
	...
	validateNoBatch := func(t *testing.T, testBatch *voucher.Batch, st *mock.BatchStore) {
		t.Helper()
	...
	validateBatch := func(t *testing.T, testBatch *voucher.Batch, st *mock.BatchStore) {
		t.Helper()
	...
	t.Run("expect put create put error", func(t *testing.T) {
		...
		testAmount := vouchertesting.NewBigInt()
...
func TestBatchServiceUpdateBlockNumber(t *testing.T) {
	testChainState := &voucher.ChainState{
		Block:        1,
...
func newTestStoreAndServiceWithListener(
	t *testing.T,
	owner []byte,
	batchListener voucher.BatchEventListener,
	opts ...mock.Option,
) (voucher.EventUpdater, *mock.BatchStore, storage.StateStorer) {
	t.Helper()
	s := mocks.NewStateStore()
	store := mock.New(opts...)
...
func newTestStoreAndService(t *testing.T, opts ...mock.Option) (voucher.EventUpdater, *mock.BatchStore, storage.StateStorer) {
	t.Helper()
	return newTestStoreAndServiceWithListener(t, nil, nil, opts...)
}

func createBatch(t *testing.T, store voucher.Storer, b *voucher.Batch) {
	t.Helper()
...
func putChainState(t *testing.T, store voucher.Storer, cs *voucher.ChainState) {
	t.Helper()
