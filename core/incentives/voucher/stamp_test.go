...
func TestStampMarshalling(t *testing.T) {
	sExp := vouchertesting.MustNewStamp()
	buf, _ := sExp.MarshalBinary()
	if len(buf) != voucher.StampSize {
		t.Fatalf("invalid length for serialised stamp. expected %d, got  %d", voucher.StampSize, len(buf))
	}
	s := voucher.NewStamp(nil, nil, nil, nil)
...
func compareStamps(t *testing.T, s1, s2 *voucher.Stamp) {
...
func TestStampIndexMarshalling(t *testing.T) {
	...
	index := voucher.IndexToBytes(expBucket, expIndex)
	bucket, idx := voucher.BytesToIndex(index)
	if bucket != expBucket {
...
func TestValidStamp(t *testing.T) {
	...
	b := vouchertesting.MustNewBatch(vouchertesting.WithOwner(owner))
	bs := mock.New(mock.WithBatch(b))
	signer := crypto.NewDefaultSigner(privKey)
	issuer := voucher.NewStampIssuer("label", "keyID", b.ID, big.NewInt(3), b.Depth, b.BucketDepth, 1000, true)
	stamper := voucher.NewStamper(issuer, signer)
	...
	ch, err = voucher.ValidStamp(bs)(ch, stBytes)
	if err != nil {
		t.Fatal(err)
	}

	compareStamps(t, st, ch.Stamp().(*voucher.Stamp))
