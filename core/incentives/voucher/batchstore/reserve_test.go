...
func TestPutChainState(t *testing.T) {
	...
	for _, tc := range tcs {
		...
		for _, c := range tc.chain {
			...
			// update chain state
			err := store.PutChainState(&voucher.ChainState{
				Block:        c.block,
...
func TestUnreserveAndLowerStorageRadius(t *testing.T) {
	...
	err = store.Iterate(func(b *voucher.Batch) (bool, error) {
...
func TestBatchExpiry(t *testing.T) {
	...
	batch := vouchertest.MustNewBatch(
		vouchertest.WithValue(int64(4)),
		vouchertest.WithDepth(0),
		vouchertest.WithStart(111),
	)
	if err := store.Save(batch); err != nil {
		t.Fatal(err)
	}

	esi := voucher.NewStampIssuer("", "", batch.ID, big.NewInt(3), 11, 10, 1000, true)
...
func TestUnexpiredBatch(t *testing.T) {
	...
	batch := vouchertest.MustNewBatch(
		vouchertest.WithValue(int64(14)),
		vouchertest.WithDepth(0),
		vouchertest.WithStart(111),
	)
	if err := store.Save(batch); err != nil {
	...
	esi := voucher.NewStampIssuer("", "", batch.ID, big.NewInt(15), 11, 10, 1000, true)
	emp := mockpost.New(mockpost.WithIssuer(esi))
	store.SetBatchExpiryHandler(emp)
...
func setupBatchStore(t *testing.T) voucher.Storer {
	t.Helper()
	dir := t.TempDir()
	...
	err = bStore.PutChainState(&voucher.ChainState{
		Block:        0,
...
func checkState(t *testing.T, name string, store voucher.Storer, radius uint8) {
...
func addBatch(t *testing.T, s voucher.Storer, depth uint8, value int) *voucher.Batch {

	t.Helper()

	batch := vouchertest.MustNewBatch(
		vouchertest.WithValue(int64(value)),
		vouchertest.WithDepth(depth),
		vouchertest.WithStart(111),
	)
