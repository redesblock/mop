...
// booking represents an bookkeeper action and the expected result afterwards
...
func TestMutex(t *testing.T) {
	...
	t.Run("locked mutex takes context into account", func(t *testing.T) {
		...
		ctx, cancel := context.WithTimeout(context.Background(), time.Millisecond)
		defer cancel()
		if err := m.TryLock(ctx); !errors.Is(err, bookkeeper.ErrFailToLock) {
			t.Errorf("expected %v, got %v", bookkeeper.ErrFailToLock, err)
		}
	})
}

// TestAccountingAddBalance does several bookkeeper actions and verifies the balance after each steep
func TestAccountingAddBalance(t *testing.T) {
	...
	acc, err := bookkeeper.NewAccounting(testPaymentThreshold, testPaymentTolerance, testPaymentEarly, logger, store, pricing, big.NewInt(testRefreshRate), testLightFactor, p2pmock.New())
	if err != nil {
		t.Fatal(err)
	}

	peer1Addr, err := cluster.ParseHexAddress("00112233")
	if err != nil {
		t.Fatal(err)
	}

	peer2Addr, err := cluster.ParseHexAddress("00112244")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAccountingAddOriginatedBalance(t *testing.T) {
	...
	acc, err := bookkeeper.NewAccounting(testPaymentThreshold, testPaymentTolerance, testPaymentEarly, logger, store, pricing, big.NewInt(0), testLightFactor, p2pmock.New())
	if err != nil {
		t.Fatal(err)
	}

	f := func(ctx context.Context, peer cluster.Address, amount *big.Int) {
	...
	for i, booking := range bookings {
		...
		makePayFunc := func(currentBooking bookingT) func(ctx context.Context, peer cluster.Address, amount *big.Int) {
			return func(ctx context.Context, peer cluster.Address, amount *big.Int) {
...
// It creates an bookkeeper instance, does some bookkeeper
// Then it creates a new bookkeeper instance with the same store and verifies the balances
...
func TestAccountingReserve(t *testing.T) {
	...
	if !errors.Is(err, bookkeeper.ErrOverdraft) {
...
func TestAccountingReserveAheadOfTime(t *testing.T) {
	...
	pay := func(ctx context.Context, peer cluster.Address, amount *big.Int) {
	}
...
func TestAccountingCallSettlementMonetary(t *testing.T) {
	...
	acc.SetRefreshFunc(func(ctx context.Context, peer cluster.Address, amount *big.Int) {
		acc.NotifyRefreshmentSent(peer, amount, amount, ts*1000, 0, nil)
		refreshchan <- paymentCall{peer: peer, amount: amount}
	})

	acc.SetPayFunc(func(ctx context.Context, peer cluster.Address, amount *big.Int) {
		acc.NotifyPaymentSent(peer, amount, nil)
...
func TestAccountingCallSettlementEarly(t *testing.T) {
	...
	acc, err := bookkeeper.NewAccounting(testPaymentThreshold, testPaymentTolerance, earlyPayment, logger, store, pricing, big.NewInt(testRefreshRate), testLightFactor, p2pmock.New())
	if err != nil {
		t.Fatal(err)
	}
...
func TestAccountingSurplusBalance(t *testing.T) {
	...
	acc, err := bookkeeper.NewAccounting(testPaymentThreshold, 0, 0, logger, store, pricing, big.NewInt(testRefreshRate), testLightFactor, p2pmock.New())
	if err != nil {
		t.Fatal(err)
	}
	peer1Addr, err := cluster.ParseHexAddress("00112233")
...
func (p *pricingMock) AnnouncePaymentThreshold(ctx context.Context, peer cluster.Address, paymentThreshold *big.Int) error {
	p.called = true
	p.peer = peer
...
func TestAccountingConnected(t *testing.T) {
	...
	_, err := bookkeeper.NewAccounting(testPaymentThreshold, testPaymentTolerance, testPaymentEarly, logger, store, pricing, big.NewInt(testRefreshRate), testLightFactor, p2pmock.New())
	if err != nil {
		t.Fatal(err)
	}
...
func TestAccountingNotifyPaymentThreshold(t *testing.T) {
	...
	acc, err := bookkeeper.NewAccounting(testPaymentThreshold, testPaymentTolerance, 0, logger, store, pricing, big.NewInt(testRefreshRate), testLightFactor, p2pmock.New())
	if err != nil {
		t.Fatal(err)
	}
...
func TestAccountingPeerDebt(t *testing.T) {
	...
	peer1Addr := cluster.MustParseHexAddress("00112233")
	acc.Connect(peer1Addr, true)
	...
	peer2Addr := cluster.MustParseHexAddress("11112233")
	acc.Connect(peer2Addr, true)
	...
	peer3Addr := cluster.MustParseHexAddress("22112233")
...
func TestAccountingCallPaymentErrorRetries(t *testing.T) {
	...
	acc, err := bookkeeper.NewAccounting(testPaymentThreshold, testPaymentTolerance, testPaymentEarly, logger, store, pricing, big.NewInt(1), testLightFactor, p2pmock.New())
	if err != nil {
		t.Fatal(err)
	}
...
func TestAccountingGhostOverdraft(t *testing.T) {
	...
	f := func(s cluster.Address, t time.Duration, reason string) error {
	...
	acc, err := bookkeeper.NewAccounting(testPaymentThreshold, testPaymentTolerance, testPaymentEarly, logger, store, pricing, big.NewInt(testRefreshRate), testLightFactor, p2pmock.New(p2pmock.WithBlocklistFunc(f)))
	if err != nil {
		t.Fatal(err)
	}
	...
	peer, err := cluster.ParseHexAddress("00112233")
	if err != nil {
		t.Fatal(err)
	}
...
func testAccountingSettlementGrowingThresholds(t *testing.T, settleFunc func(t *testing.T, acc *bookkeeper.Accounting, peer1Addr cluster.Address, debitRefresh int64), fullNode bool, testPayThreshold *big.Int, testGrowth int64) {
	...
	for j := 0; j < 18; j++ {
		...
		// Check increase happened (meaning pricing AnnounceThreshold was called by bookkeeper)
		if pricing.paymentThreshold.Cmp(checkPaymentThreshold) != 0 {
			t.Fatalf("expected threshold %v got %v", checkPaymentThreshold, pricing.paymentThreshold)
		}
...
func debitAndRefresh(t *testing.T, acc *bookkeeper.Accounting, peer1Addr cluster.Address, debitRefresh int64) {
	// Create debt
...
func debitAndReceivePayment(t *testing.T, acc *bookkeeper.Accounting, peer1Addr cluster.Address, debitRefresh int64) {
	// Create debt
	debitAction, err := acc.PrepareDebit(context.Background(), peer1Addr, uint64(debitRefresh))
	if err != nil {
		t.Fatal(err)
