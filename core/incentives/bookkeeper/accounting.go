// Package bookkeeper provides functionalities needed
// to do per-peer bookkeeper.
package bookkeeper
...
type Interface interface {
	...
	PrepareCredit(ctx context.Context, peer cluster.Address, price uint64, originated bool) (Action, error)
	// PrepareDebit returns an bookkeeper Action for the later debit to be executed on and to implement shadowing a possibly credited part of reserve on the other side.
	PrepareDebit(ctx context.Context, peer cluster.Address, price uint64) (Action, error)
	// Balance returns the current balance for the given peer.
	Balance(peer cluster.Address) (*big.Int, error)
	// SurplusBalance returns the current surplus balance for the given peer.
	SurplusBalance(peer cluster.Address) (*big.Int, error)
	...
	CompensatedBalance(peer cluster.Address) (*big.Int, error)
...
// Action represents an bookkeeper action that can be applied
...
type PayFunc func(context.Context, cluster.Address, *big.Int)

// RefreshFunc is the function used for chainsync time-based settlement
type RefreshFunc func(context.Context, cluster.Address, *big.Int)

// Mutex is a drop in replacement for the chainsync.Mutex
...
// accountingPeer holds all in-memory bookkeeper information for one peer.
type accountingPeer struct {
	lock                           *Mutex   // lock to be held during any bookkeeper action for this peer
...
// Accounting is the main implementation of the bookkeeper interface.
...
func (a *Accounting) getIncreasedExpectedDebt(peer cluster.Address, accountingPeer *accountingPeer, bigPrice *big.Int) (*big.Int, *big.Int, error) {
...
func (a *Accounting) PrepareCredit(ctx context.Context, peer cluster.Address, price uint64, originated bool) (Action, error) {
...
// called.
func (a *Accounting) settle(peer cluster.Address, balance *accountingPeer) error {
	now := a.timeNow()
...
// Balance returns the current balance for the given peer.
func (a *Accounting) Balance(peer cluster.Address) (balance *big.Int, err error) {
...
func (a *Accounting) OriginatedBalance(peer cluster.Address) (balance *big.Int, err error) {
...
func (a *Accounting) SurplusBalance(peer cluster.Address) (balance *big.Int, err error) {
...
func (a *Accounting) CompensatedBalance(peer cluster.Address) (compensated *big.Int, err error) {
...
func peerBalanceKey(peer cluster.Address) string {
...
func peerSurplusBalanceKey(peer cluster.Address) string {
	return fmt.Sprintf("%s%s", balancesSurplusPrefix, peer.String())
}

func originatedBalanceKey(peer cluster.Address) string {
	return fmt.Sprintf("%s%s", balancesOriginatedPrefix, peer.String())
}

// getAccountingPeer returns the accountingPeer for a given cluster address.
// If not found in memory it will initialize it.
func (a *Accounting) getAccountingPeer(peer cluster.Address) *accountingPeer {
...
func (a *Accounting) notifyPaymentThresholdUpgrade(peer cluster.Address, accountingPeer *accountingPeer) {
...
func (a *Accounting) PeerAccounting() (map[string]PeerInfo, error) {
	...
	for peer, accountingPeer := range accountingPeersList {
		...
		peerAddress := cluster.MustParseHexAddress(peer)
...
func balanceKeyPeer(key []byte) (cluster.Address, error) {
	k := string(key)

	split := strings.SplitAfter(k, balancesPrefix)
	if len(split) != 2 {
		return cluster.ZeroAddress, errors.New("no peer in key")
	}
...
func surplusBalanceKeyPeer(key []byte) (cluster.Address, error) {
	k := string(key)
...
func (a *Accounting) PeerDebt(peer cluster.Address) (*big.Int, error) {
	accountingPeer := a.getAccountingPeer(peer)
...
func (a *Accounting) peerLatentDebt(peer cluster.Address) (*big.Int, error) {
...
func (a *Accounting) shadowBalance(peer cluster.Address, accountingPeer *accountingPeer) (shadowBalance *big.Int, err error) {
	balance := new(big.Int)
	zero := big.NewInt(0)
...
func (a *Accounting) NotifyPaymentSent(peer cluster.Address, amount *big.Int, receivedError error) {
	loggerV1 := a.logger.V(1).Register()
...
// NotifyPaymentThreshold should be called to notify bookkeeper of changes in the payment threshold
func (a *Accounting) NotifyPaymentThreshold(peer cluster.Address, paymentThreshold *big.Int) error {
	accountingPeer := a.getAccountingPeer(peer)
...
func (a *Accounting) NotifyPaymentReceived(peer cluster.Address, amount *big.Int) error {
	loggerV1 := a.logger.V(1).Register()
...
func (a *Accounting) NotifyRefreshmentSent(peer cluster.Address, attemptedAmount, amount *big.Int, timestamp int64, allegedInterval int64, receivedError error) {
	accountingPeer := a.getAccountingPeer(peer)
	...
	if err != nil {
		a.logger.Warning("bookkeeper: notifyrefreshmentsent failed to decrease originated balance: %v", err)
	}

}

// NotifyRefreshmentReceived is called by pseudosettle when we receive a time based settlement.
func (a *Accounting) NotifyRefreshmentReceived(peer cluster.Address, amount *big.Int, timestamp int64) error {
	loggerV1 := a.logger.V(1).Register()
...
func (a *Accounting) PrepareDebit(ctx context.Context, peer cluster.Address, price uint64) (Action, error) {
	loggerV1 := a.logger.V(1).Register()
...
func (a *Accounting) increaseBalance(peer cluster.Address, _ *accountingPeer, price *big.Int) (*big.Int, error) {
	loggerV1 := a.logger.V(1).Register()
...
func (a *Accounting) blocklistUntil(peer cluster.Address, multiplier int64) (int64, error) {
...
func (a *Accounting) blocklist(peer cluster.Address, multiplier int64, reason string) error {
...
func (a *Accounting) Connect(peer cluster.Address, fullNode bool) {
	accountingPeer := a.getAccountingPeer(peer)
	zero := big.NewInt(0)
...
func (a *Accounting) decreaseOriginatedBalanceTo(peer cluster.Address, limit *big.Int) error {
	loggerV1 := a.logger.V(1).Register()
...
func (a *Accounting) decreaseOriginatedBalanceBy(peer cluster.Address, amount *big.Int) error {
	loggerV1 := a.logger.V(1).Register()
...
func (a *Accounting) Disconnect(peer cluster.Address) {
	accountingPeer := a.getAccountingPeer(peer)
