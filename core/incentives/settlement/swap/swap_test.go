...
import (
	...
	mockchequestore "github.com/redesblock/mop/core/incentives/settlement/swap/chequestore/mock"
	"github.com/redesblock/mop/core/incentives/settlement/swap/swapprotocol"
	"github.com/redesblock/mop/core/log"
	mockstore "github.com/redesblock/mop/core/storer/statestore/mock"
)

type swapProtocolMock struct {
	emitCheque func(context.Context, cluster.Address, common.Address, *big.Int, swapprotocol.IssueFunc) (*big.Int, error)
}

func (m *swapProtocolMock) EmitCheque(ctx context.Context, peer cluster.Address, beneficiary common.Address, value *big.Int, issueFunc swapprotocol.IssueFunc) (*big.Int, error) {
	if m.emitCheque != nil {
...
func (t *testObserver) PeerDebt(peer cluster.Address) (*big.Int, error) {
	return nil, nil
}

func (t *testObserver) NotifyPaymentReceived(peer cluster.Address, amount *big.Int) error {
...
func (t *testObserver) NotifyRefreshmentSent(peer cluster.Address, attemptedAmount, amount *big.Int, timestamp int64, allegedInterval int64, receivedError error) {
}

func (t *testObserver) NotifyRefreshmentReceived(peer cluster.Address, amount *big.Int, time int64) error {
	return nil
}

func (t *testObserver) NotifyPaymentSent(peer cluster.Address, amount *big.Int, err error) {
...
func (t *testObserver) Connect(peer cluster.Address, full bool) {

}

func (t *testObserver) Disconnect(peer cluster.Address) {

}

type addressbookMock struct {
	migratePeer     func(oldPeer, newPeer cluster.Address) error
	beneficiary     func(peer cluster.Address) (beneficiary common.Address, known bool, err error)
	chequebook      func(peer cluster.Address) (chequebookAddress common.Address, known bool, err error)
	beneficiaryPeer func(beneficiary common.Address) (peer cluster.Address, known bool, err error)
	chequebookPeer  func(chequebook common.Address) (peer cluster.Address, known bool, err error)
	putBeneficiary  func(peer cluster.Address, beneficiary common.Address) error
	putChequebook   func(peer cluster.Address, chequebook common.Address) error
	addDeductionFor func(peer cluster.Address) error
	addDeductionBy  func(peer cluster.Address) error
	getDeductionFor func(peer cluster.Address) (bool, error)
	getDeductionBy  func(peer cluster.Address) (bool, error)
}

func (m *addressbookMock) MigratePeer(oldPeer, newPeer cluster.Address) error {
	return m.migratePeer(oldPeer, newPeer)
}
func (m *addressbookMock) Beneficiary(peer cluster.Address) (beneficiary common.Address, known bool, err error) {
	return m.beneficiary(peer)
}
func (m *addressbookMock) Chequebook(peer cluster.Address) (chequebookAddress common.Address, known bool, err error) {
	return m.chequebook(peer)
}
func (m *addressbookMock) BeneficiaryPeer(beneficiary common.Address) (peer cluster.Address, known bool, err error) {
	return m.beneficiaryPeer(beneficiary)
}
func (m *addressbookMock) ChequebookPeer(chequebook common.Address) (peer cluster.Address, known bool, err error) {
	return m.chequebookPeer(chequebook)
}
func (m *addressbookMock) PutBeneficiary(peer cluster.Address, beneficiary common.Address) error {
	return m.putBeneficiary(peer, beneficiary)
}
func (m *addressbookMock) PutChequebook(peer cluster.Address, chequebook common.Address) error {
	return m.putChequebook(peer, chequebook)
}
func (m *addressbookMock) AddDeductionFor(peer cluster.Address) error {
	return m.addDeductionFor(peer)
}
func (m *addressbookMock) AddDeductionBy(peer cluster.Address) error {
	return m.addDeductionBy(peer)
}
func (m *addressbookMock) GetDeductionFor(peer cluster.Address) (bool, error) {
	return m.getDeductionFor(peer)
}
func (m *addressbookMock) GetDeductionBy(peer cluster.Address) (bool, error) {
...
func TestReceiveCheque(t *testing.T) {
	...
	peer := cluster.MustParseHexAddress("abcd")
	...
	addressbook := &addressbookMock{
		chequebook: func(p cluster.Address) (common.Address, bool, error) {
			if !peer.Equal(p) {
				t.Fatal("querying chequebook for wrong peer")
			}
			return chequebookAddress, true, nil
		},
		putChequebook: func(p cluster.Address, chequebook common.Address) (err error) {
			if !peer.Equal(p) {
		...
		addDeductionFor: func(p cluster.Address) error {
...
func TestPay(t *testing.T) {
	...
	networkID := uint64(1)
	addressbook := &addressbookMock{
		beneficiary: func(p cluster.Address) (common.Address, bool, error) {
			if !peer.Equal(p) {
	...
	swap := swap.New(
		&swapProtocolMock{
			emitCheque: func(ctx context.Context, p cluster.Address, b common.Address, a *big.Int, issueFunc swapprotocol.IssueFunc) (*big.Int, error) {
				if !peer.Equal(p) {
...
func TestPayIssueError(t *testing.T) {
	...
	swap := swap.New(
		&swapProtocolMock{
			emitCheque: func(c context.Context, a1 cluster.Address, a2 common.Address, i *big.Int, issueFunc swapprotocol.IssueFunc) (*big.Int, error) {
				return nil, errReject
			},
		},
		logger,
...
func TestHandshake(t *testing.T) {
	...
	peer, err := crypto.NewOverlayFromBSCAddress(beneficiary[:], networkID, txHash.Bytes())

	if err != nil {
		t.Fatalf("crypto.NewOverlayFromBSCAddress(...): unexpected error: %v", err)
	}
	...
	swapService := swap.New(
		...
		&addressbookMock{
			...
			beneficiaryPeer: func(common.Address) (peer cluster.Address, known bool, err error) {
				return peer, true, nil
			},
			migratePeer: func(oldPeer, newPeer cluster.Address) error {
				return nil
			},
			putBeneficiary: func(p cluster.Address, b common.Address) error {
				putCalled = true
				return nil
			},
		},
...
func TestHandshakeNewPeer(t *testing.T) {
	...
	networkID := uint64(1)
	peer, err := crypto.NewOverlayFromBSCAddress(beneficiary[:], networkID, trx.Bytes())
	...
	swapService := swap.New(
		...
		&addressbookMock{
			...
			beneficiaryPeer: func(beneficiary common.Address) (cluster.Address, bool, error) {
				return peer, true, nil
			},
			migratePeer: func(oldPeer, newPeer cluster.Address) error {
				return nil
...
func TestMigratePeer(t *testing.T) {
	...
	swapService := swap.New(
		...
		&addressbookMock{
			beneficiaryPeer: func(beneficiary common.Address) (cluster.Address, bool, error) {
				return cluster.MustParseHexAddress("00112233"), true, nil
			},
			migratePeer: func(oldPeer, newPeer cluster.Address) error {
				return nil
			},
...
func TestStateStoreKeys(t *testing.T) {
	...
	clusterAddress := cluster.MustParseHexAddress("deff")

	expected := "swap_chequebook_peer_deff"
	if swap.PeerKey(clusterAddress) != expected {
		t.Fatalf("wrong peer key. wanted %s, got %s", expected, swap.PeerKey(clusterAddress))
	}
	...
	if swap.PeerBeneficiaryKey(clusterAddress) != expected {
		t.Fatalf("wrong peer beneficiary key. wanted %s, got %s", expected, swap.PeerBeneficiaryKey(clusterAddress))
	}
	...
	if swap.PeerDeductedByKey(clusterAddress) != expected {
		t.Fatalf("wrong peer deducted by key. wanted %s, got %s", expected, swap.PeerDeductedByKey(clusterAddress))
	}

	expected = "swap_deducted_for_peer_deff"
	if swap.PeerDeductedForKey(clusterAddress) != expected {
		t.Fatalf("wrong peer deducted for key. wanted %s, got %s", expected, swap.PeerDeductedForKey(clusterAddress))
	}
}
