...
var (
	...
	// ErrChequeValueTooLow is the error returned if the after deduction value of a cheque did not cover 1 bookkeeper credit
...
// RecoverCheque recovers the issuer BNB Smart Chain from a signed cheque
func RecoverCheque(cheque *SignedCheque, chaindID int64) (common.Address, error) {
	...
	ethAddr, err := crypto.NewBSCAddress(*pubkey)
	if err != nil {
		return common.Address{}, err
	}
