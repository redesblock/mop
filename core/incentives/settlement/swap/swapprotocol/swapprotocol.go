...
import (
	...
	"github.com/redesblock/mop/core/incentives/settlement/swap/swapprotocol/pb"
...
type Interface interface {
	...
	EmitCheque(ctx context.Context, peer cluster.Address, beneficiary common.Address, amount *big.Int, issue IssueFunc) (balance *big.Int, err error)
}

// Swap is the interface the settlement layer should implement to receive cheques.
type Swap interface {
	// ReceiveCheque is called by the swap protocol if a cheque is received.
	ReceiveCheque(ctx context.Context, peer cluster.Address, cheque *chequebook.SignedCheque, exchangeRate, deduction *big.Int) error
	// Handshake is called by the swap protocol when a handshake is received.
	Handshake(peer cluster.Address, beneficiary common.Address) error
	GetDeductionForPeer(peer cluster.Address) (bool, error)
	GetDeductionByPeer(peer cluster.Address) (bool, error)
	AddDeductionByPeer(peer cluster.Address) error
}
...
func (s *Service) init(ctx context.Context, p p2p.Peer) error {
	beneficiary := common.BytesToAddress(p.BSCAddress)
...
func (s *Service) headler(receivedHeaders p2p.Headers, peerAddress cluster.Address) (returnHeaders p2p.Headers) {
...
func (s *Service) EmitCheque(ctx context.Context, peer cluster.Address, beneficiary common.Address, amount *big.Int, issue IssueFunc) (balance *big.Int, err error) {
