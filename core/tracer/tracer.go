...
// contextKey is used to reference a tracer context span as context value.
...
// Tracer connect to a tracer server and handles tracer spans and contexts
...
// StartSpanFromContext starts a new tracer span that is either a root one or a
...
// AddContextHeader adds a tracer span context to provided p2p Headers from
// the go context. If the tracer span context is not present in go context,
...
// FromHeaders returns tracer span context from p2p Headers. If the tracer
...
// WithContextFromHeaders returns a new context with injected tracer span
// context if they are found in p2p Headers. If the tracer span context is not
...
// AddContextHTTPHeader adds a tracer span context to provided HTTP headers
// from the go context. If the tracer span context is not present in
...
// FromHTTPHeaders returns tracer span context from HTTP headers. If the tracer
// span context is not present in go context, ErrContextNotFound is returned.
...
// WithContextFromHTTPHeaders returns a new context with injected tracer span
// context if they are found in HTTP headers. If the tracer span context is not
// present in go context, ErrContextNotFound is returned.
...
// FromContext return tracer span context from go context. If the tracer span
...
// exists in tracer span context stored from go context.
