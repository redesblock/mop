...
func NewSimpleManifestReference(ref cluster.Address, l file.LoadSaver) (Interface, error) {
	m := &simpleManifest{
		manifest:  simple.NewManifest(),
		reference: ref,
		ls:        l,
...
func (m *simpleManifest) Lookup(_ context.Context, path string) (Entry, error) {
	...
	address, err := cluster.ParseHexAddress(n.Reference())
	if err != nil {
		return nil, fmt.Errorf("parse cluster address: %w", err)
	}
...
func (m *simpleManifest) Store(ctx context.Context, storeSizeFn ...StoreSizeFunc) (cluster.Address, error) {
	data, err := m.manifest.MarshalBinary()
	if err != nil {
		return cluster.ZeroAddress, fmt.Errorf("manifest marshal error: %w", err)
	}
	...
	if len(storeSizeFn) > 0 {
		...
		for i := range storeSizeFn {
			...
			if err != nil {
				return cluster.ZeroAddress, fmt.Errorf("manifest store size func: %w", err)
			}
		}
	}
	...
	if err != nil {
		return cluster.ZeroAddress, fmt.Errorf("manifest save error: %w", err)
	}
	m.reference = cluster.NewAddress(ref)
...
func (m *simpleManifest) IterateAddresses(ctx context.Context, fn AddressIterFunc) error {
	...
	walker := func(path string, entry simple.Entry, err error) error {
		...
		ref, err := cluster.ParseHexAddress(entry.Reference())
		if err != nil {
			return err
		}
...
func (m *simpleManifest) load(ctx context.Context, reference cluster.Address) error {
