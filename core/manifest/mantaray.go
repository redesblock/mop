...
func (m *mantarayManifest) Lookup(ctx context.Context, path string) (Entry, error) {
	...
	address := cluster.NewAddress(node.Entry())
...
func (m *mantarayManifest) Store(ctx context.Context, storeSizeFn ...StoreSizeFunc) (cluster.Address, error) {
	...
	address := cluster.NewAddress(m.trie.Reference())
...
func (m *mantarayManifest) IterateAddresses(ctx context.Context, fn cluster.AddressIterFunc) error {
	...
	emptyAddr := cluster.NewAddress([]byte{31: 0})
	walker := func(path []byte, node *mantaray.Node, err error) error {
		...
		if node != nil {
			...
			if node.IsValueType() && len(node.Entry()) > 0 {
				entry := cluster.NewAddress(node.Entry())
				...
				// (e.g.: during the unpin traverser operation
