...
// interface.
func (db *DB) Put(ctx context.Context, mode storage.ModePut, chs ...flock.Chunk) (exist []bool, err error) {
...
func (db *DB) put(mode storage.ModePut, chs ...flock.Chunk) (exist []bool, err error) {
...
func (db *DB) putRequest(batch *leveldb.Batch, binIDs map[uint8]uint64, item shed.Item, forcePin, forceCache bool) (exists bool, gcSizeChange int64, err error) {
	...
	if err != nil {
		...
		if err != nil {
			...
			if db.po(flock.NewAddress(item.Address)) >= radius.Radius {
	...
	item.BinID, err = db.incBinID(binIDs, db.po(flock.NewAddress(item.Address)))
	if err != nil {
		return false, 0, err
	}
