...
//
// # go test -benchmem -run=none github.com/ethersphere/flock/storage/localstore -bench BenchmarkRetrievalIndexes -v
// goos: darwin
// goarch: amd64
...
func benchmarkRetrievalIndexes(b *testing.B, o *Options, count int) {
	b.StopTimer()
	db := newTestDB(b, o)
	addrs := make([]flock.Address, count)
	for i := 0; i < count; i++ {
...
//
// go test -benchmem -run=none github.com/ethersphere/flock/storage/localstore -bench BenchmarkUpload -v
// goos: darwin
// goarch: amd64
// pkg: github.com/ethersphere/flock/storage/localstore
