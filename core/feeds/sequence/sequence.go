...
func (f *finder) At(ctx context.Context, at, after int64) (ch cluster.Chunk, current, next feeds.Index, err error) {
	for i := uint64(0); ; i++ {
...
type result struct {
	chunk    cluster.Chunk // the chunk found
...
func (f *asyncFinder) At(ctx context.Context, at, after int64) (ch cluster.Chunk, cur, next feeds.Index, err error) {
...
func (f *asyncFinder) asyncGet(ctx context.Context, at int64, index uint64) <-chan cluster.Chunk {
	c := make(chan cluster.Chunk)
	go func() {
		defer close(c)
...
func (f *asyncFinder) get(ctx context.Context, at int64, idx uint64) (cluster.Chunk, error) {
