...
import (
	...
	testingsoc "github.com/redesblock/mop/core/chunk/soc/testing"
...
const (
	...
	defaultMediaType = "mop-manifest-mantaray"
)
...
// targeting cluster.AddressIterFunc execution.
...
// Next matches the signature of cluster.AddressIterFunc needed in
// Traverser.Traverse method and collects statistics about it's execution.
func (i *addressIterator) Next(addr cluster.Address) error {
	i.mu.Lock()
	defer i.mu.Unlock()
...
func TestTraversalBytes(t *testing.T) {
	testCases := []struct {
		...
		{
			dataSize:      cluster.ChunkSize * 128,
			wantHashCount: 129,
			wantHashes: []string{
		...
		{
			dataSize:      cluster.ChunkSize * 129,
			wantHashCount: 131,
			wantHashes: []string{
		...
		{
			dataSize:      cluster.ChunkSize*129 - 1,
			wantHashCount: 131,
			wantHashes: []string{
	...
	for _, tc := range testCases {
		chunkCount := int(math.Ceil(float64(tc.dataSize) / cluster.ChunkSize))
		t.Run(fmt.Sprintf("%d-chunk-%d-bytes", chunkCount, tc.dataSize), func(t *testing.T) {
			...
			err = traverser.New(storerMock).Traverse(ctx, address, iter.Next)
			if err != nil {
				t.Fatal(err)
			}
...
func TestTraversalFiles(t *testing.T) {
	...
	for _, tc := range testCases {
		chunkCount := int(math.Ceil(float64(tc.filesSize) / cluster.ChunkSize))
		t.Run(fmt.Sprintf("%d-chunk-%d-bytes", chunkCount, tc.filesSize), func(t *testing.T) {
			...
			err = fManifest.Add(ctx, "/", manifest.NewEntry(cluster.ZeroAddress, rootMtdt))
			if err != nil {
				t.Fatal(err)
			}
...
func TestTraversalSOC(t *testing.T) {
	...
	s := testingsoc.GenerateMockSOC(t, generateSample(cluster.ChunkSize))
	sch := s.Chunk()
	...
	err = traverser.New(store).Traverse(ctx, sch.Address(), iter.Next)
	if err != nil {
		t.Fatal(err)
	}
