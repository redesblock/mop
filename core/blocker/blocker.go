...
type Blocker struct {
	...
	blocklistCallback func(cluster.Address)
}

func New(blocklister p2p.Blocklister, flagTimeout, blockDuration, wakeUpTime time.Duration, callback func(cluster.Address), logger log.Logger) *Blocker {
...
func (b *Blocker) Flag(addr cluster.Address) {
	if b.blocklister.NetworkStatus() != p2p.NetworkStatusAvailable {
		return
	}
...
func (b *Blocker) Unflag(addr cluster.Address) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *Blocker) PruneUnseen(seen []cluster.Address) {
