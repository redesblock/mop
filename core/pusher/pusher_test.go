...
func TestSendChunkToSyncWithTag(t *testing.T) {
	...
	mtags, p, storer := createPusher(t, triggerPeer, pushSyncService, defaultMockvalidVouch, mock.WithClosestPeer(closestPeer), mock.WithNeighborhoodDepth(0))
	defer storer.Close()
	defer p.Close()
...
func TestSendChunkToPushSyncWithoutTag(t *testing.T) {
	...
	_, p, storer := createPusher(t, triggerPeer, pushSyncService, defaultMockvalidVouch, mock.WithClosestPeer(closestPeer), mock.WithNeighborhoodDepth(0))
	defer storer.Close()
	defer p.Close()
...
func TestSendChunkAndReceiveInvalidReceipt(t *testing.T) {
	...
	_, p, storer := createPusher(t, triggerPeer, pushSyncService, defaultMockvalidVouch, mock.WithClosestPeer(closestPeer))
	defer storer.Close()
	defer p.Close()
...
func TestPusherRetryShallow(t *testing.T) {
	...
	*pusher.RetryCount = 3
	...
	_, ps, storer := createPusher(t, pivotPeer, pushSyncService, defaultMockvalidVouch, mock.WithClosestPeer(closestPeer), mock.WithNeighborhoodDepth(31))
	defer ps.Close()
...
func TestChunkWithInvalidVouchSkipped(t *testing.T) {
	...
	_, p, storer := createPusher(t, triggerPeer, pushSyncService, validVouch, mock.WithClosestPeer(closestPeer), mock.WithNeighborhoodDepth(0))
	defer storer.Close()
	defer p.Close()
...
func createPusher(t *testing.T, addr swarm.Address, pushSyncService pushsync.PushSyncer, validVouch postage.ValidVouchFn, mockOpts ...mock.Option) (*tags.Tags, *pusher.Service, *Store) {
	...
	pusherService := pusher.New(1, pusherStorer, peerSuggester, pushSyncService, validVouch, mtags, logger, nil, 0)
