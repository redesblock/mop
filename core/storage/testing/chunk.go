...
import (
	...
	flocktesting "github.com/redesblock/mop/core/flock/test"
	postagetesting "github.com/redesblock/mop/core/postage/testing"
)

var mockVouch flock.Vouch
...
var fixtureChunks = map[string]flock.Chunk{
	"0025": flock.NewChunk(
		flock.MustParseHexAddress("0025737be11979e91654dffd2be817ac1e52a2dadb08c97a7cef12f937e707bc"),
	...
	"0033": flock.NewChunk(
		flock.MustParseHexAddress("0033153ac8cfb0c343db1795f578c15ed8ef827f3e68ed3c58329900bf0d7276"),
	...
	"02c2": flock.NewChunk(
		flock.MustParseHexAddress("02c2bd0db71efb7d245eafcc1c126189c1f598feb80e8f14e7ecef913c6a2ef5"),
	...
	"7000": flock.NewChunk(
		flock.MustParseHexAddress("70002115a015d40a1f5ef68c29d072f06fae58854934c1cb399fcb63cf336127"),
...
func GenerateTestRandomChunk() flock.Chunk {
...
// addressed chunk.
func GenerateTestRandomInvalidChunk() flock.Chunk {
	data := make([]byte, flock.ChunkSize)
	_, _ = rand.Read(data)
	key := make([]byte, flock.SectionSize)
	_, _ = rand.Read(key)
	vouch := postagetesting.MustNewVouch()
	return flock.NewChunk(flock.NewAddress(key), data).WithVouch(vouch)
}

// GenerateTestRandomChunks generates a slice of random
// Chunks by using GenerateTestRandomChunk function.
func GenerateTestRandomChunks(count int) []flock.Chunk {
...
func GenerateTestRandomChunkAt(target flock.Address, po int) flock.Chunk {
	data := make([]byte, flock.ChunkSize)
	_, _ = rand.Read(data)
	addr := flocktesting.RandomAddressAt(target, po)
	vouch := postagetesting.MustNewVouch()
	return flock.NewChunk(addr, data).WithVouch(vouch)
}

// FixtureChunk gets a pregenerated content-addressed chunk and
// panics if one is not found.
func FixtureChunk(prefix string) flock.Chunk {
