...
func (c *Client) Resolve(name string) (Address, error) {
	if c.resolveFn == nil {
		return cluster.ZeroAddress, fmt.Errorf("resolveFn: %w", errNotImplemented)
	}

	hash, err := c.resolveFn(c.registry, common.HexToAddress(c.contractAddr), name)
	if err != nil {
		return cluster.ZeroAddress, fmt.Errorf("%v: %w", err, ErrResolveFailed)
	}

	// Ensure that the content hash string is in a valid format, eg.
	// "mop://<address>".
	if !strings.HasPrefix(hash, clusterContentHashPrefix) {
		return cluster.ZeroAddress, fmt.Errorf("check content hash prefix %s: %w", hash, resolver.ErrInvalidContentHash)
	}

	// Trim the prefix and try to parse the result as a mop address.
	addr, err := cluster.ParseHexAddress(strings.TrimPrefix(hash, clusterContentHashPrefix))
	if err != nil {
		return cluster.ZeroAddress, fmt.Errorf("parse response hash %s: %w", hash, resolver.ErrInvalidContentHash)
	}
