...
func TestResolve(t *testing.T) {
	...
	testClusterAddr := cluster.MustParseHexAddress("aaabbbcc")
	...
	testCases := []struct {
		...
		{
			desc: "resolve does not prefix address with /cluster",
			resolveFn: func(*goens.Registry, common.Address, string) (string, error) {
				return testClusterAddr.String(), nil
			},
			wantErr: resolver.ErrInvalidContentHash,
		},
		{
			...
			resolveFn: func(*goens.Registry, common.Address, string) (string, error) {
				return ens.ClusterContentHashPrefix + testClusterAddr.String(), nil
			},
			wantErr: resolver.ErrInvalidContentHash,
		},
		{
