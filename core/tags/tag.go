...
const (
	...
	StateSplit               // chunk has been processed by filehasher/cluster safe call
	StateStored              // chunk stored locally
	StateSeen                // chunk previously seen
	StateSent                // chunk sent to neighbourhood
	StateSynced              // proof is received; chunk removed from chainsync db; chunk is available everywhere
)
...
type Tag struct {
	...
	Address   cluster.Address // the associated cluster hash for this tag
	StartedAt time.Time       // tag started to calculate ETA

	// end-to-end tag tracer
	ctx        context.Context     // tracer context
	span       opentracing.Span    // tracer root span
...
func NewTag(ctx context.Context, uid uint32, total int64, tracer *tracer.Tracer, stateStore storage.StateStorer, logger log.Logger) *Tag {
	t := &Tag{
		Uid:        uid,
		StartedAt:  time.Now(),
		Total:      total,
	...
	// we don't need any type of mctx Deadline or cancellation for this particular mctx
...
// DoneSplit sets total count to SPLIT count and sets the associated cluster hash for this tag
// is meant to be called when splitter finishes for input streams of unknown size
func (t *Tag) DoneSplit(address cluster.Address) (int64, error) {
	total := atomic.LoadInt64(&t.Split)
	atomic.StoreInt64(&t.Total, total)

	if !address.Equal(cluster.ZeroAddress) {
		t.Address = address
	}
...
func (tag *Tag) UnmarshalBinary(buffer []byte) error {
	...
	buffer = buffer[n:]
	if t > 0 {
		tag.Address = cluster.NewAddress(buffer[:t])
	}
