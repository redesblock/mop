...
func TestListAll(t *testing.T) {
	...
	// which in the test results in the same uids already existing in statestore that the "create few more tags" creates below in chainsync.Map
	// this highlights that upon tags.Create(), already existing values are only checked in chainsync.Map but not in statestore
	...
	// first tags are from chainsync.Map
...
func TestPersistence(t *testing.T) {
	...
	ta.Split = 10
	ta.Stored = 8
	_, err = ta.DoneSplit(cluster.ZeroAddress)
	if err != nil {
		t.Fatal(err)
	}
