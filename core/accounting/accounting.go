...
type Accounting struct {
	...
	logger            logging.Logger
	store             storage.StateStorer
	paymentThreshold  uint64 // the payment threshold in HOP we communicate to our peers
	paymentTolerance  uint64 // the amount in HOP we let peers exceed the payment threshold before disconnected
	settlement        settlement.Interface
	metrics           metrics
}
