...
func TestProofCorrectness(t *testing.T) {
	...
	pool := bmt.NewPool(bmt.NewConf(cluster.NewHasher, testSegmentCount, testSegmentSectionSize, 128))
	hh := pool.Get()
	defer pool.Put(hh)
	...
	t.Run("proof for left most", func(t *testing.T) {
		...
		if !bytes.Equal(proof.Section, testData[:2*hh.Size()]) {
	...
	t.Run("proof for right most", func(t *testing.T) {
		...
		if !bytes.Equal(proof.Section, testData[126*hh.Size():]) {
			t.Fatal("section incorrect")
		}
	...
	t.Run("proof for middle", func(t *testing.T) {
		...
		if !bytes.Equal(proof.Section, testData[64*hh.Size():66*hh.Size()]) {
			t.Fatal("section incorrect")
		}
