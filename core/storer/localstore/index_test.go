...
func TestDB_pullIndex(t *testing.T) {
	...
	testItemsOrder(t, db.pullIndex, chunks, func(i, j int) (less bool) {
		poi := cluster.Proximity(db.baseKey, chunks[i].Address().Bytes())
		poj := cluster.Proximity(db.baseKey, chunks[j].Address().Bytes())
		if poi < poj {
			return true
		}
		if poi > poj {
...
func TestDB_gcIndex(t *testing.T) {
	...
	t.Run("chainsync one chunk", func(t *testing.T) {
		ch := chunks[0]
	...
	t.Run("chainsync all chunks", func(t *testing.T) {
		for i := range chunks {
