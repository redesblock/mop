...
//
// # go test -benchmem -run=none github.com/ethersphere/cluster/storage/localstore -bench BenchmarkRetrievalIndexes -v
// goos: darwin
// goarch: amd64
...
func benchmarkRetrievalIndexes(b *testing.B, o *Options, count int) {
	b.StopTimer()
	db := newTestDB(b, o)
	addrs := make([]cluster.Address, count)
	for i := 0; i < count; i++ {
...
//
// go test -benchmem -run=none github.com/ethersphere/cluster/storage/localstore -bench BenchmarkUpload -v
// goos: darwin
// goarch: amd64
// core: github.com/ethersphere/cluster/storage/localstore
