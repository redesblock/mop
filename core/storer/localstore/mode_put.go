...
// interface.
func (db *DB) Put(ctx context.Context, mode storage.ModePut, chs ...cluster.Chunk) (exist []bool, err error) {
	...
	if err != nil {
		db.logger.Error(err, "mode_put failed", "mode", mode, "chunks", len(chs))
...
func (db *DB) put(ctx context.Context, mode storage.ModePut, chs ...cluster.Chunk) (exist []bool, retErr error) {
	...
	var (
		...
		// successfully committed due to voucherIndex collisions
	...
	putChunk := func(ch cluster.Chunk, index int, putOp func(shed.Item) (bool, int64, int64, error)) (bool, int64, int64, error) {
	...
	case storage.ModePutRequest, storage.ModePutRequestPin, storage.ModePutRequestCache:
		for i, ch := range chs {
			pin := mode == storage.ModePutRequestPin     // force pins in this mode
	...
	case storage.ModePutSync:
		for i, ch := range chs {
			...
			if err != nil {
				return nil, fmt.Errorf("put chainsync: %w", err)
			}
			exist[i] = exists
			if !exists {
...
func (db *DB) putRequest(
	...
	previous, err := db.voucherIndexIndex.Get(item)
	if err != nil {
		...
		radius, err := db.voucherRadiusIndex.Get(item)
		if err != nil {
			if !errors.Is(err, leveldb.ErrNotFound) {
				return false, 0, 0, err
			}
		} else {
			if db.po(cluster.NewAddress(item.Address)) >= radius.Radius {
				reserveSizeChange--
			}
		}
	}
	...
	err = db.voucherChunksIndex.PutInBatch(batch, item)
	if err != nil {
		return false, 0, 0, err
	}
	err = db.voucherIndexIndex.PutInBatch(batch, item)
	if err != nil {
		return false, 0, 0, err
	}
