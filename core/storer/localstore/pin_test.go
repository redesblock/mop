...
func TestPinCounter(t *testing.T) {
	...
	t.Run("+1 after first pins", func(t *testing.T) {
		...
		if pinCounter != 1 {
			t.Fatalf("want pins counter %d but got %d", 1, pinCounter)
		}
	})
	t.Run("2 after second pins", func(t *testing.T) {
		err = db.Set(ctx, storage.ModeSetPin, addr)
		if err != nil {
			t.Fatal(err)
		}
		...
		if pinCounter != 2 {
			t.Fatalf("want pins counter %d but got %d", 2, pinCounter)
		}
	})
...
func runCountsTest(t *testing.T, name string, db *DB, r, a, push, pull, pin, gc int) {
	...
	t.Run(name, func(t *testing.T) {
		...
		t.Run("pins Index count", newItemsCountTest(db.pinIndex, pin))
