...
func (db *DB) UnreserveBatch(id []byte, radius uint8) (evicted uint64, err error) {
	...
	i, err := db.voucherRadiusIndex.Get(item)
	if err != nil {
	...
	for bin := oldRadius; bin < radius; bin++ {
		...
		item.Radius = bin
		if err := db.voucherRadiusIndex.PutInBatch(batch, item); err != nil {
			return 0, err
		}
		if bin == cluster.MaxPO {
			if err := db.voucherRadiusIndex.DeleteInBatch(batch, item); err != nil {
				return 0, err
			}
		}
...
func (db *DB) unpinBatchChunks(id []byte, bin uint8) (uint64, error) {
	...
	unpin := func(item shed.Item) (stop bool, err error) {
		addr := cluster.NewAddress(item.Address)
		...
		if err != nil {
			...
			// we need to do this because a user might pins a chunk on top of
			// the reserve pins. when we unpin due to an unreserve call, then
	...
	for {
		currentBatchSize := 0
		more := false
		err := db.voucherChunksIndex.Iterate(func(item shed.Item) (bool, error) {
...
func withinRadius(db *DB, item shed.Item) bool {
	po := db.po(cluster.NewAddress(item.Address))
