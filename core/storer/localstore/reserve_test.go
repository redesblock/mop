...
func TestDB_ReserveGC_AllOutOfRadius(t *testing.T) {
	...
	t.Run("voucher chunks index count", newItemsCountTest(db.voucherChunksIndex, int(gcTarget)))

	// voucherRadiusIndex gets removed only when the batches are called with evict on MaxPO+1
	// therefore, the expected index count here is larger than one would expect.
	t.Run("voucher radius index count", newItemsCountTest(db.voucherRadiusIndex, 0))
...
func TestDB_ReserveGC_AllWithinRadius(t *testing.T) {
	...
	t.Run("voucher chunks index count", newItemsCountTest(db.voucherChunksIndex, chunkCount))
...
func TestDB_ReserveGC_Unreserve(t *testing.T) {
	...
	t.Run("voucher chunks index count", newItemsCountTest(db.voucherChunksIndex, chunkCount+90-10))

	// voucherRadiusIndex gets removed only when the batches are called with evict on MaxPO+1
	// therefore, the expected index count here is larger than one would expect.
	t.Run("voucher radius index count", newItemsCountTest(db.voucherRadiusIndex, chunkCount))
...
// cluster.MaxPO+1 results in the correct behaviour.
func TestDB_ReserveGC_EvictMaxPO(t *testing.T) {
	...
	unres := func(f voucher.UnreserveIteratorFn) error {
		...
		for i = 0; i < len(batchIDs); i++ {
			...
			stop, err = f(item, cluster.MaxPO+1)
			if err != nil {
				return err
			}
			if stop {
...
func TestReserveSize(t *testing.T) {
	...
	t.Run("variadic put chainsync", func(t *testing.T) {
		var (
			db = newTestDB(t, &Options{
				Capacity:        100,
				ReserveCapacity: 100,
	...
	t.Run("variadic put upload then set chainsync", func(t *testing.T) {
		var (
			db = newTestDB(t, &Options{
				Capacity:        100,
				ReserveCapacity: 100,
	...
	t.Run("sequencial put chainsync", func(t *testing.T) {
		var (
			db = newTestDB(t, &Options{
				Capacity:        100,
				ReserveCapacity: 100,
	...
	t.Run("sequencial put upload then set chainsync", func(t *testing.T) {
		var (
			db = newTestDB(t, &Options{
				Capacity:        100,
				ReserveCapacity: 100,
...
func TestDB_ReserveGC_BatchedUnreserve(t *testing.T) {
	...
	genChunk := func() cluster.Chunk {
		newStamp := vouchertesting.MustNewBatchStamp(stamp.BatchID())
		ch := generateTestRandomChunkAt(cluster.NewAddress(db.baseKey), 2)
	...
	t.Run("voucher chunks index count", newItemsCountTest(db.voucherChunksIndex, 90))

	// we use the same batch for all chunks
	t.Run("voucher radius index count", newItemsCountTest(db.voucherRadiusIndex, 1))
