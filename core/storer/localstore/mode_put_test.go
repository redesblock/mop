	...
	for _, tc := range multiChunkTestCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			t.Run("first put", func(t *testing.T) {
				...
				newItemsCountTest(db.voucherIndexIndex, tc.count)(t)
...
func TestModePutRequestPin(t *testing.T) {
	...
	for _, tc := range multiChunkTestCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			newItemsCountTest(db.voucherChunksIndex, tc.count)(t)
			newItemsCountTest(db.voucherIndexIndex, tc.count)(t)
			// gc index should be always 0 since we're pins
	...
	for _, tc := range multiChunkTestCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for i := 0; i < tc.count; i++ {
				chunk := generateTestRandomChunkAt(cluster.NewAddress(db.baseKey), 2)
...
func TestModePutSyncUpload_SameIndex(t *testing.T) {
	...
	newItemsCountTest(db.voucherIndexIndex, 1)(t)
	newIndexGCSizeTest(db)(t)
}
...
func TestModePutUpload_parallel(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			chunksChan := make(chan []cluster.Chunk)
...
func TestModePut_SameStamp(t *testing.T) {
	...
	for _, modeTc1 := range putModes {
		for _, modeTc2 := range putModes {
			for _, tc := range []struct {
				persistChunk cluster.Chunk
				discardChunk cluster.Chunk
			}{
				{
				...
				t.Run(modeTc1.String()+modeTc2.String(), func(t *testing.T) {
					...
					newItemsCountTest(db.retrievalDataIndex, 1)(t)
					newItemsCountTest(db.voucherChunksIndex, 1)(t)
					newItemsCountTest(db.voucherRadiusIndex, 1)(t)
					newItemsCountTest(db.voucherIndexIndex, 1)(t)
...
func generateChunkWithTimestamp(stamp *voucher.Stamp, timestamp int64) cluster.Chunk {
	tsBuf := make([]byte, 8)
...
func generateImmutableChunkWithTimestamp(stamp *voucher.Stamp, timestamp int64) cluster.Chunk {
...
//
// # go test -benchmem -run=none github.com/ethersphere/cluster/storage/localstore -bench BenchmarkPutUpload -v
//
// goos: darwin
// goarch: amd64
// core: github.com/ethersphere/cluster/storage/localstore
