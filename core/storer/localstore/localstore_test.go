...
func TestParallelPutAndGet(t *testing.T) {
	...
	chunks := make([]cluster.Chunk, 0, chunkCount)
...
func chunkAddresses(chunks []cluster.Chunk) []cluster.Address {
	addrs := make([]cluster.Address, len(chunks))
...
func TestGenerateTestRandomChunk(t *testing.T) {
	...
	dataLen := len(c1.Data())
	if dataLen != cluster.ChunkSize+cluster.SpanSize {
		t.Errorf("first chunk data length %v, want %v", dataLen, cluster.ChunkSize)
	}
	...
	dataLen = len(c2.Data())
	if dataLen != cluster.ChunkSize+cluster.SpanSize {
		t.Errorf("second chunk data length %v, want %v", dataLen, cluster.ChunkSize)
	}
...
func newRetrieveIndexesTest(db *DB, chunk cluster.Chunk, storeTimestamp, accessTimestamp int64) func(t *testing.T) {
...
func newRetrieveIndexesTestWithAccess(db *DB, ch cluster.Chunk, storeTimestamp, accessTimestamp int64) func(t *testing.T) {
	return func(t *testing.T) {
		t.Helper()
...
func newPullIndexTest(db *DB, ch cluster.Chunk, binID uint64, wantError error) func(t *testing.T) {
	return func(t *testing.T) {
		t.Helper()
...
func newPushIndexTest(db *DB, ch cluster.Chunk, storeTimestamp int64, wantError error) func(t *testing.T) {
	return func(t *testing.T) {
		t.Helper()
...
func newGCIndexTest(db *DB, chunk cluster.Chunk, storeTimestamp, accessTimestamp int64, binID uint64, wantError error, stamp *voucher.Stamp) func(t *testing.T) {
	return func(t *testing.T) {
		t.Helper()
...
// chunk values are in the pins index.
func newPinIndexTest(db *DB, chunk cluster.Chunk, wantError error) func(t *testing.T) {
	return func(t *testing.T) {
		t.Helper()
...
func validateItem(t *testing.T, item shed.Item, address []byte, storeTimestamp, accessTimestamp int64, stamp cluster.Stamp) {
	t.Helper()
...
func TestDBDebugIndexes(t *testing.T) {
	...
	// set the chunk for pins and expect the index count to grow
	...
	// assert that there's a pins and gc exclude entry now
