...
func testDBCollectGarbageWorker(t *testing.T) {
	...
	addrs := make([]cluster.Address, chunkCount)
	...
	t.Run("voucher index count", newItemsCountTest(db.voucherIndexIndex, int(gcTarget)))
...
func TestPinGC(t *testing.T) {
	...
	addrs := make([]cluster.Address, 0)
	pinAddrs := make([]cluster.Address, 0)
	...
	t.Run("pins Index count", newItemsCountTest(db.pinIndex, pinChunksCount))

	t.Run("pull index count", newItemsCountTest(db.pullIndex, int(gcTarget)+pinChunksCount))

	t.Run("voucher index count", newItemsCountTest(db.voucherIndexIndex, int(gcTarget)+pinChunksCount))
		...
		err := db.gcIndex.Iterate(func(item shed.Item) (stop bool, err error) {
			for _, pinHash := range pinAddrs {
				if bytes.Equal(pinHash.Bytes(), item.Address) {
					t.Fatal("pins chunk present in gcIndex")
				}
			}
			return false, nil
		}, nil)
...
// Upload chunks, pins those chunks, add to GC after it is pinned
...
func TestGCAfterPin(t *testing.T) {
	...
	t.Run("pins Index count", newItemsCountTest(db.pinIndex, chunkCount))

	t.Run("gc index count", newItemsCountTest(db.gcIndex, int(0)))

	t.Run("voucher index count", newItemsCountTest(db.voucherIndexIndex, chunkCount))
...
func TestDB_collectGarbageWorker_withRequests(t *testing.T) {
	...
	// upload and chainsync another chunk to trigger
	// garbage collection
	ch := generateTestRandomChunk()
	// call unreserve on the batch with radius 0 so that
	// localstore is aware of the batch and the chunk can
...
func TestPinAfterMultiGC(t *testing.T) {
	...
	pinnedChunks := make([]cluster.Address, 0)
	...
	t.Run("pins Index count", newItemsCountTest(db.pinIndex, len(pinnedChunks)))
	...
	for _, addr := range pinnedChunks {
		...
		gotChunk, err := db.Get(context.Background(), storage.ModeGetRequest, cluster.NewAddress(outItem.Address))
		if err != nil {
			t.Fatal(err)
		}
		if !gotChunk.Address().Equal(cluster.NewAddress(addr.Bytes())) {
...
func generateAndPinAChunk(t *testing.T, db *DB) cluster.Chunk {
	// Create a chunk and pins it
	ch := generateTestRandomChunk()
...
func addRandomChunks(t *testing.T, count int, db *DB, pin bool) []cluster.Chunk {
	var chunks []cluster.Chunk
	for i := 0; i < count; i++ {
		ch := generateTestRandomChunk()
		unreserveChunkBatch(t, db, 0, ch)
...
func unreserveChunkBatch(t *testing.T, db *DB, radius uint8, chs ...cluster.Chunk) {
	t.Helper()
	for _, ch := range chs {
...
func TestReserveEvictionWorker(t *testing.T) {
	...
	t.Run("voucher radius count", newItemsCountTest(db.voucherRadiusIndex, 1))
	...
	for i := 0; i < chunkCount-1; i++ {
		ch := generateTestRandomChunkAt(cluster.NewAddress(db.baseKey), 3).WithBatch(2, 3, 2, false)
		_, err := db.Put(context.Background(), storage.ModePutUpload, ch)
		if err != nil {
			t.Fatal(err)
		}
