...
type DB struct {
	...
	// stateStore is needed to access the pins Service.Pins() method.
	...
	voucherChunksIndex shed.Index

	// voucher radius index
	voucherRadiusIndex shed.Index

	// voucher index index
	voucherIndexIndex shed.Index
	...
	dirtyAddresses []cluster.Address
...
func safeInit(rootPath, sharkyBasePath string, db *DB) error {
	...
	recoverySharky, err := sharky.NewRecovery(sharkyBasePath, sharkyNoOfShards, cluster.SocMaxChunkSize)
	if err != nil {
		return err
	}
...
func New(path string, baseKey []byte, ss storage.StateStorer, o *Options, logger log.Logger) (db *DB, err error) {
	...
	capacityMB := float64((db.cacheCapacity+uint64(batchstore.Capacity))*cluster.ChunkSize) * 9.5367431640625e-7
	...
	db.sharky, err = sharky.New(sharkyBase, sharkyNoOfShards, cluster.SocMaxChunkSize)
	if err != nil {
		return nil, err
	}
	...
	db.pullIndex, err = db.shed.NewIndex("PO|BinID->Hash", shed.IndexFuncs{
		EncodeKey: func(fields shed.Item) (key []byte, err error) {
			key = make([]byte, 9)
			key[0] = db.po(cluster.NewAddress(fields.Address))
	...
	// Create a index structure for storing pinned chunks and their pins counts
	...
	db.voucherChunksIndex, err = db.shed.NewIndex("BatchID|PO|Hash->nil", shed.IndexFuncs{
		EncodeKey: func(fields shed.Item) (key []byte, err error) {
			key = make([]byte, 65)
			copy(key[:32], fields.BatchID)
			key[32] = db.po(cluster.NewAddress(fields.Address))
	...
	db.voucherRadiusIndex, err = db.shed.NewIndex("BatchID->Radius", shed.IndexFuncs{
		EncodeKey: func(fields shed.Item) (key []byte, err error) {
			key = make([]byte, 32)
			copy(key[:32], fields.BatchID)
			return key, nil
	...
	db.voucherIndexIndex, err = db.shed.NewIndex("BatchID|BatchIndex->Hash|Timestamp", shed.IndexFuncs{
		EncodeKey: func(fields shed.Item) (key []byte, err error) {
			key = make([]byte, 40)
			copy(key[:32], fields.BatchID)
...
func (db *DB) po(addr cluster.Address) (bin uint8) {
	return cluster.Proximity(db.baseKey, addr.Bytes())
}
...
func (db *DB) DebugIndices() (indexInfo map[string]int, err error) {
	...
	for k, v := range map[string]shed.Index{
		...
		"voucherChunksIndex":   db.voucherChunksIndex,
		"voucherRadiusIndex":   db.voucherRadiusIndex,
		"voucherIndexIndex":    db.voucherIndexIndex,
	} {
...
func (db *DB) stateStoreHasPins() (bool, error) {
	pins, err := pins.NewService(nil, db.stateStore, nil).Pins()
	if err != nil {
		return false, err
	}
...
func chunkToItem(ch cluster.Chunk) shed.Item {
	return shed.Item{
...
func addressToItem(addr cluster.Address) shed.Item {
	return shed.Item{
		Address: addr.Bytes(),
	}
}
...
func addressesToItems(addrs ...cluster.Address) []shed.Item {
