...
import (
	...
	"github.com/redesblock/mop/core/chain/transaction/wrapped"
...
// InitChain will initialize the BNB Smart Chain backend at the given endpoint and
...
func InitChain(
	...
	if err != nil {
		return nil, common.Address{}, 0, nil, nil, fmt.Errorf("BNB Smart Chain address: %w", err)
	}

	transactionMonitor := transaction.NewMonitor(logger, backend, overlayBSCAddress, pollingInterval, cancellationDepth)
	...
	return backend, overlayBSCAddress, chainID.Int64(), transactionMonitor, transactionService, nil
}
...
func InitChequebookService(
	...
	if deployGasPrice != "" {
		...
		ctx = mctx.SetGasPrice(ctx, gasPrice)
	}
...
func (m noOpChainBackend) CodeAt(context.Context, common.Address, *big.Int) ([]byte, error) {
	return common.FromHex(mabi.SimpleSwapFactoryDeployedBinv0_1_0), nil
}
