	...
	for _, c := range []struct {
	...
	}{
		{
			...
			chunk: func() flock.Chunk {
				...
				wrongAddress := flock.NewAddress(wrongAddressBytes)
				return flock.NewChunk(wrongAddress, sch.Data())
			},
		},
		{
			name: "invalid data",
			chunk: func() flock.Chunk {
				...
				chunkData[0] = 0x01
				return flock.NewChunk(socAddress, data)
			},
		},
		{
			name: "invalid id",
		...
		},
		{
			name: "nil data",
			chunk: func() flock.Chunk {
				return flock.NewChunk(socAddress, nil)
			},
		},
		{
			name: "small data",
			chunk: func() flock.Chunk {
				return flock.NewChunk(socAddress, []byte("small"))
			},
		},
		{
			name: "large data",
			chunk: func() flock.Chunk {
				return flock.NewChunk(socAddress, []byte(strings.Repeat("a", flock.ChunkSize+flock.SpanSize+1)))
			},
		},
	} {
