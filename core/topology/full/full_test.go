...
func TestAddPeers(t *testing.T) {
	...
	hopAddr, err := hop.NewAddress(crypto.NewDefaultSigner(pk), underlay, overlay, 1)
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Run("OK - no connected peers", func(t *testing.T) {
		...
		p2p := p2pmock.New(p2pmock.WithConnectFunc(func(_ context.Context, addr ma.Multiaddr) (*hop.Address, error) {
		...
		if err := ab.Put(overlay, *hopAddr); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("OK - connected peers - peer already connected", func(t *testing.T) {
		...
		addrAlreadyConnected, err := hop.NewAddress(crypto.NewDefaultSigner(pk), underlay, alreadyConnected, 1)
		if err != nil {
			t.Fatal(err)
		}
