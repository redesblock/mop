...
import (
	...
	mockk "github.com/redesblock/mop/core/p2p/topology/kademlia/mock"
	mockps "github.com/redesblock/mop/core/protocol/pullsync/mock"
...
func TestSyncFlow_PeerWithinDepth_Live(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		expCalls     []c    // expected historical chainsync calls
		expLiveCalls []c    // expected live chainsync calls
	}{
		{
...
func TestSyncFlow_PeerWithinDepth_Historical(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// check historical chainsync calls
...
// When a chainsync reply is specified with block=true, the protocol mock basically
...
func TestDepthChange(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			cursors:        []uint64{0, 0, 0, 0, 0},
			binsNotSyncing: []uint8{1, 2}, // only bins 3,4 are expected to chainsync
...
func checkIntervals(t *testing.T, s storage.StateStorer, addr cluster.Address, expInterval string, bin uint8) {
	t.Helper()
...
func checkNotFound(t *testing.T, s storage.StateStorer, addr cluster.Address, bin uint8) {
	t.Helper()
	key := puller.PeerIntervalKey(addr, bin)
	i := &intervalstore.Intervals{}
	err := s.Get(key, i)
...
func waitCheckCalls(t *testing.T, expCalls []c, callsFn func(cluster.Address) []mockps.SyncCall, addr cluster.Address) {
	t.Helper()
	for i := 0; i < 10; i++ {
		time.Sleep(50 * time.Millisecond)
		calls := callsFn(addr)
...
func waitCursorsCalled(t *testing.T, ps *mockps.PullSyncMock, addr cluster.Address, invert bool) {
	t.Helper()
	for i := 0; i < 20; i++ {
		if v := ps.CursorsCalls(addr); v {
			if invert {
				t.Fatal("got a call to chainsync to a peer but shouldnt")
			} else {
				return
			}
		}
...
func waitLiveSyncCalled(t *testing.T, ps *mockps.PullSyncMock, addr cluster.Address, invert bool) {
	t.Helper()
	for i := 0; i < 15; i++ {
	...
	t.Fatal("timed out waiting for chainsync")
}

// waitSyncCalled waits until SyncInterval is called on the address given.
func waitSyncCalled(t *testing.T, ps *mockps.PullSyncMock, addr cluster.Address, invert bool) {
	t.Helper()
	for i := 0; i < 15; i++ {
		v := ps.SyncCalls(addr)
		if len(v) > 0 {
...
func waitSyncCalledTimes(t *testing.T, ps *mockps.PullSyncMock, addr cluster.Address, times int) {
	t.Helper()
	for i := 0; i < 15; i++ {
		v := ps.SyncCalls(addr)
		if len(v) == times {
...
func waitLiveSyncCalledTimes(t *testing.T, ps *mockps.PullSyncMock, addr cluster.Address, times int) {
	t.Helper()
	for i := 0; i < 15; i++ {
		v := ps.LiveSyncCalls(addr)
		if len(v) == times {
...
func newPuller(ops opts) (*puller.Puller, storage.StateStorer, *mockk.Mock, *mockps.PullSyncMock) {
	...
	rs := &reserveStateGetter{rs: voucher.ReserveState{StorageRadius: ops.syncRadius}}
...
func (r *reserveStateGetter) GetReserveState() *voucher.ReserveState {
	return &r.rs
}
