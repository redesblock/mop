...
import (
	...
	"github.com/redesblock/mop/core/storer/intervalstore"
...
type Puller struct {
	...
	reserveState voucher.ReserveStateGetter
...
func New(stateStore storage.StateStorer, topology topology.Driver, reserveState voucher.ReserveStateGetter, pullSync pullsync.Interface, logger log.Logger, o Options, warmupTime time.Duration) *Puller {
	var (
		bins uint8 = cluster.MaxBins
	)
	if o.Bins != 0 {
		bins = o.Bins
	}
...
func (p *Puller) manage(warmupTime time.Duration) {
	...
	for {
		...
		case <-c:
			...
			_ = p.topology.EachPeerRev(func(peerAddr cluster.Address, po uint8) (stop, jumpToNext bool, err error) {
				if po >= neighborhoodDepth {
					bp := p.syncPeers[po]
					// delete from peersDisconnected since we'd like to chainsync
					// with this peer
					delete(peersDisconnected, peerAddr.ByteString())

					// within depth, chainsync everything
...
func (p *Puller) disconnectPeer(peer cluster.Address, po uint8) {
...
func (p *Puller) recalcPeer(ctx context.Context, peer cluster.Address, po, d uint8) (dontSync bool) {
	loggerV2 := p.logger.V(2).Register()
...
func (p *Puller) syncPeer(ctx context.Context, peer cluster.Address, po, d uint8) {
	loggerV2 := p.logger.V(2).Register()
...
func (p *Puller) syncPeerBin(ctx context.Context, syncCtx *syncPeer, peer cluster.Address, bin uint8, cur uint64) {
...
func (p *Puller) histSyncWorker(ctx context.Context, peer cluster.Address, bin uint8, cur uint64) {
	loggerV2 := p.logger.V(2).Register()
	...
	for {
		...
		if err != nil {
			...
			// chainsync operations, the context can be expired here, causing us
...
func (p *Puller) liveSyncWorker(ctx context.Context, peer cluster.Address, bin uint8, cur uint64) {
	loggerV2 := p.logger.V(2).Register()
	...
	for {
		...
		if err != nil {
			loggerV2.Debug("liveSyncWorker exit on chainsync error", "peer_address", peer, "bin", bin, "from", from, "error", err)
			if ruid == 0 {
...
func (p *Puller) addPeerInterval(peer cluster.Address, bin uint8, start, end uint64) (err error) {
...
func (p *Puller) nextPeerInterval(peer cluster.Address, bin uint8) (start, end uint64, empty bool, err error) {
...
func (p *Puller) getOrCreateInterval(peer cluster.Address, bin uint8) (*intervalstore.Intervals, error) {
...
func peerIntervalKey(peer cluster.Address, bin uint8) string {
...
type syncPeer struct {
	...
	binCancelFuncs map[uint8]func() // slice of context cancel funcs for historical chainsync. index is bin

	sync.Mutex
}

func newSyncPeer(addr cluster.Address, bins uint8) *syncPeer {
	return &syncPeer{
		address:        addr,
...
// called when peer disconnects or on shutdown, cleans up ongoing chainsync operations
...
func isSyncing(p *Puller, addr cluster.Address) bool {
