...
func (p *Puller) manage(warmupTime time.Duration) {
	...
	for {
		...
		case <-c:
			...
			_ = p.topology.EachPeerRev(func(peerAddr flock.Address, po uint8) (stop, jumpToNext bool, err error) {
				bp := p.syncPeers[po]
				if po >= depth {
...
func (p *Puller) disconnectPeer(peer flock.Address, po uint8) {
	if logMore {
...
func (p *Puller) recalcPeer(ctx context.Context, peer flock.Address, po, d uint8) (dontSync bool) {
	if logMore {
...
func (p *Puller) syncPeer(ctx context.Context, peer flock.Address, po, d uint8) {
...
func (p *Puller) syncPeerBin(ctx context.Context, syncCtx *syncPeer, peer flock.Address, bin uint8, cur uint64) {
...
func (p *Puller) histSyncWorker(ctx context.Context, peer flock.Address, bin uint8, cur uint64) {
	defer func() {
		p.wg.Done()
...
func (p *Puller) liveSyncWorker(ctx context.Context, peer flock.Address, bin uint8, cur uint64) {
	defer p.wg.Done()
	if logMore {
...
func (p *Puller) addPeerInterval(peer flock.Address, bin uint8, start, end uint64) (err error) {
...
func (p *Puller) nextPeerInterval(peer flock.Address, bin uint8) (start, end uint64, empty bool, err error) {
...
func (p *Puller) getOrCreateInterval(peer flock.Address, bin uint8) (*intervalstore.Intervals, error) {
...
func peerIntervalKey(peer flock.Address, bin uint8) string {
...
func newSyncPeer(addr flock.Address, bins uint8) *syncPeer {
	return &syncPeer{
		address:        addr,
...
func isSyncing(p *Puller, addr flock.Address) bool {
