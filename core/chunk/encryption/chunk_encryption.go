...
func newSpanEncryption(key Key) Interface {
	refSize := int64(cluster.HashSize + KeyLength)
	return New(key, 0, uint32(cluster.ChunkSize/refSize), sha3.NewLegacyKeccak256)
}

func newDataEncryption(key Key) Interface {
	return New(key, int(cluster.ChunkSize), 0, sha3.NewLegacyKeccak256)
}
