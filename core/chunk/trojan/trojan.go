...
import (
	...
	"github.com/redesblock/mop/core/chunk/encryption/elgamal"
...
func Wrap(ctx context.Context, topic Topic, msg []byte, recipient *ecdsa.PublicKey, targets Targets) (cluster.Chunk, error) {
	...
	enc, ephpub, err := elgamal.NewEncryptor(recipient, topic[:], 4032, cluster.NewHasher)
	if err != nil {
		return nil, err
	}
	...
	f := func(nonce []byte) (cluster.Chunk, error) {
		hash, err := h(nonce)
		if err != nil {
			return nil, err
		}
		...
		chunk := cluster.NewChunk(cluster.NewAddress(hash), append(hint, append(nonce, payload...)...))
		return chunk, nil
	}
...
func mine(ctx context.Context, odd bool, f func(nonce []byte) (cluster.Chunk, error)) (cluster.Chunk, error) {
	...
	result := make(chan cluster.Chunk, 8)
	for i := 0; i < 8; i++ {
		eg.Go(func() error {
			nonce := make([]byte, 32)
...
func matchTopic(key *ecdsa.PrivateKey, pubkey *ecdsa.PublicKey, hint, topic []byte) (encryption.Decrypter, error) {
	dec, err := elgamal.NewDecrypter(key, pubkey, topic, cluster.NewHasher)
	if err != nil {
		return nil, err
	}
