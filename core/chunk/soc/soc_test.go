...
func TestNew(t *testing.T) {
	...
	spanBytes := make([]byte, cluster.SpanSize)
	binary.LittleEndian.PutUint64(spanBytes, uint64(len(payload)))
	if !bytes.Equal(chunkData[:cluster.SpanSize], spanBytes) {
		t.Fatalf("span mismatch. got %x want %x", chunkData[:cluster.SpanSize], spanBytes)
	}

	if !bytes.Equal(chunkData[cluster.SpanSize:], payload) {
		t.Fatalf("payload mismatch. got %x want %x", chunkData[cluster.SpanSize:], payload)
	}
}
...
func TestChunk(t *testing.T) {
	...
	expectedSOCAddress := cluster.NewAddress(sum)
	...
	cursor := 0
	if !bytes.Equal(chunkData[cursor:cluster.HashSize], id) {
		t.Fatalf("id mismatch. got %x want %x", chunkData[cursor:cluster.HashSize], id)
	}
	cursor += cluster.HashSize

	signature := chunkData[cursor : cursor+cluster.SocSignatureSize]
	...
	spanBytes := make([]byte, cluster.SpanSize)
	binary.LittleEndian.PutUint64(spanBytes, uint64(len(payload)))
	if !bytes.Equal(chunkData[cursor:cursor+cluster.SpanSize], spanBytes) {
		t.Fatalf("span mismatch. got %x want %x", chunkData[cursor:cursor+cluster.SpanSize], spanBytes)
	}
	cursor += cluster.SpanSize
...
func TestSign(t *testing.T) {
	...
	owner, err := crypto.NewBSCAddress(*publicKey)
	if err != nil {
		t.Fatal(err)
	}
...
func TestFromChunk(t *testing.T) {
	...
	chunkAddress := cluster.MustParseHexAddress("2387e8e7d8a48c2a9339c97c1dc3461a9a7aa07e994c5cb8b38fd7c1b3e6ea48")
	ch := cluster.NewChunk(chunkAddress, chunkData)
