...
func TestNewCAC(t *testing.T) {
	...
	address := cluster.MustParseHexAddress(bmtHashOfData)

	expectedSpan := make([]byte, cluster.SpanSize)
...
func TestChunkInvariants(t *testing.T) {
	chunkerFunc := []struct {
		name    string
		chunker func(data []byte) (cluster.Chunk, error)
	}{
		{
			name:    "new cac",
			chunker: cac.New,
	...
	for _, f := range chunkerFunc {
		for _, cc := range []struct {
			...
			},
			{
				...
				data:    []byte(strings.Repeat("a", cluster.ChunkSize+cluster.SpanSize+1)),
...
func TestValid(t *testing.T) {
	...
	address := cluster.MustParseHexAddress(bmtHashOfFoo)

	foo := "foo"
	fooLength := len(foo)
	fooBytes := make([]byte, cluster.SpanSize+fooLength)
	binary.LittleEndian.PutUint64(fooBytes, uint64(fooLength))
	copy(fooBytes[cluster.SpanSize:], foo)
	ch := cluster.NewChunk(address, fooBytes)
	if !cac.Valid(ch) {
